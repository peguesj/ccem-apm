defmodule Apm.Plugins.Memory.ClaudeMemBridgeTest do
  @moduledoc """
  ...
  Run with: mix test test/apm/plugins/memory/claude_mem_bridge_test.exs
  """
  ...
  defp start_fixture_bridge do
    Application.put_env(:apm, :claude_mem_db_path, @fixture_db)

    try do
      {:ok, pid} = GenServer.start_link(ClaudeMemBridge, [], name: ClaudeMemBridge)
      pid
    after
      Application.delete_env(:apm, :claude_mem_db_path)
    end
  end

  # Helper: start bridge against missing DB
  defp start_missing_bridge do
    Application.put_env(:apm, :claude_mem_db_path, @missing_db)
