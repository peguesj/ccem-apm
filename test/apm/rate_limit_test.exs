defmodule Apm.RateLimitTest do
  @moduledoc """
  Tests for Apm.RateLimit (Hammer 7.x ETS sliding window) and the
  two wrapper modules Apm.Auth.RateLimiter and Apm.Proxy.RateLimiter.
  ...
  # ---------------------------------------------------------------------------
  # Apm.RateLimit (Hammer core)
  # ---------------------------------------------------------------------------

  describe "Apm.RateLimit" do
    test "first hit returns {:allow, 1}" do
      key = unique_key("core-first")
      assert {:allow, 1} = Apm.RateLimit.hit(key, :timer.seconds(60), 5)
    end
    ...
    test "hits within limit are allowed and count increments" do
      ...
      for expected_count <- 1..5 do
        assert {:allow, ^expected_count} = Apm.RateLimit.hit(key, :timer.seconds(60), 5)
      end
    end
    ...
    test "sixth hit beyond limit returns {:deny, retry_after_ms}" do
      ...
      for _ <- 1..5 do
        Apm.RateLimit.hit(key, :timer.seconds(60), 5)
      end

      assert {:deny, retry_after_ms} = Apm.RateLimit.hit(key, :timer.seconds(60), 5)
    ...
    test "short-window entries expire and allow again" do
      ...
      # Fill the bucket
      for _ <- 1..3, do: Apm.RateLimit.hit(key, window_ms, 3)
      assert {:deny, _} = Apm.RateLimit.hit(key, window_ms, 3)

      # Wait for the window to pass
      Process.sleep(window_ms + 50)

      # Should be allowed again
      assert {:allow, _} = Apm.RateLimit.hit(key, window_ms, 3)
    end
    ...
    test "different keys are isolated" do
      ...
      for _ <- 1..5, do: Apm.RateLimit.hit(key_a, :timer.seconds(60), 5)
      assert {:deny, _} = Apm.RateLimit.hit(key_a, :timer.seconds(60), 5)

      # key_b untouched — must still be allowed
      assert {:allow, 1} = Apm.RateLimit.hit(key_b, :timer.seconds(60), 5)
    end
  end

  # ---------------------------------------------------------------------------
  # Apm.Auth.RateLimiter wrapper
  # ---------------------------------------------------------------------------

  describe "Apm.Auth.RateLimiter.check/2" do
    test "returns :ok when within default :low limit (200/60s)" do
      assert :ok = Apm.Auth.RateLimiter.check(unique_key("auth-ok"), "Bash")
    end
    ...
    test "returns {:error, :rate_limited, retry_after_ms} after limit exceeded" do
      ...
      Apm.Auth.RateLimiter.configure(tool, 2, 60)

      assert :ok = Apm.Auth.RateLimiter.check(user, tool)
      assert :ok = Apm.Auth.RateLimiter.check(user, tool)

      assert {:error, :rate_limited, retry_after_ms} =
               Apm.Auth.RateLimiter.check(user, tool)
    ...
    test "record/2 is a no-op (does not double-count)" do
      ...
      Apm.Auth.RateLimiter.configure("Bash_noop", 2, 60)

      :ok = Apm.Auth.RateLimiter.check(user, "Bash_noop")
      # record is a no-op — should not consume the second slot
      :ok = Apm.Auth.RateLimiter.record(user, "Bash_noop")

      # second check should still succeed (bucket only used 1 of 2)
      assert :ok = Apm.Auth.RateLimiter.check(user, "Bash_noop")
    end

    test "configure/3 sets per-tool limits" do
      tool = unique_key("auth-configure-tool")
      Apm.Auth.RateLimiter.configure(tool, 1, 60)
      assert %{max_calls: 1, window_seconds: 60} = Apm.Auth.RateLimiter.get_tool_config(tool)
    end

    test "get_tool_config/1 returns :low defaults for unknown tools" do
      assert %{max_calls: 200, window_seconds: 60} =
               Apm.Auth.RateLimiter.get_tool_config("UnknownTool_#{System.unique_integer()}")
    end

    test "default_limits/0 returns all five risk levels" do
      limits = Apm.Auth.RateLimiter.default_limits()
    ...
    test "stats/0 returns a list (empty in this implementation)" do
      assert is_list(Apm.Auth.RateLimiter.stats())
    end
  end

  # ---------------------------------------------------------------------------
  # Apm.Proxy.RateLimiter wrapper
  # ---------------------------------------------------------------------------

  describe "Apm.Proxy.RateLimiter.allow?/2" do
    test "returns true for new scope/key pair" do
      assert Apm.Proxy.RateLimiter.allow?(unique_key("proxy-scope"), "key1")
    end
    ...
    test "returns false after default limit (100) reached" do
      ...
      for _ <- 1..100 do
        Apm.Proxy.RateLimiter.allow?(scope, "k")
      end

      refute Apm.Proxy.RateLimiter.allow?(scope, "k")
    end

    test "reset/1 is a no-op and does not crash" do
      assert :ok = Apm.Proxy.RateLimiter.reset(unique_key("proxy-reset"))
    end

    test "check/2 returns map with allowed/remaining/window_ms" do
      scope = unique_key("proxy-check")
      result = Apm.Proxy.RateLimiter.check(scope, "key")
