defmodule Apm.Auth.OpaPoliciesTest do
  @moduledoc """
  ...
  alias Apm.Auth.OpaClient

  @policies_dir Path.join(:code.priv_dir(:apm), "policies")

  setup do
    bypass = Bypass.open()
    url = "http://localhost:#{bypass.port}"
    Application.put_env(:apm, Apm.Auth.OpaClient, base_url: url, timeout_ms: 2_000)

    on_exit(fn ->
      Application.delete_env(:apm, Apm.Auth.OpaClient)
    end)
  ...
  test "live OPA: time_of_day policy evaluates with real OPA sidecar" do
    ...
    Application.put_env(:apm, Apm.Auth.OpaClient, base_url: "http://localhost:8181")
