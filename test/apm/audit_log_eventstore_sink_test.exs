defmodule Apm.AuditLog.Sinks.EventStoreSinkTest do
  @moduledoc """
  ...
  config :apm, :audit_backend is :eventstore (opt-in, default :ets).
  ...
  alias Apm.AuditLog.Sinks.EventStoreSink
  ...
  describe "behaviour contract" do
    test "EventStoreSink implements Apm.AuditLog.Sink" do
      behaviours =
  ...
  describe "default backend (:ets)" do
    setup do
      Application.delete_env(:apm, :audit_backend)
      on_exit(fn -> Application.delete_env(:apm, :audit_backend) end)
    end
    ...
    test "push_event/1 does NOT call event store adapter when backend is :ets" do
      ...
      Application.put_env(:apm, :event_store_adapter_fn, spy_adapter)
      on_exit(fn -> Application.delete_env(:apm, :event_store_adapter_fn) end)
  ...
  describe "eventstore backend" do
    setup do
      Application.put_env(:apm, :audit_backend, :eventstore)
      on_exit(fn -> Application.delete_env(:apm, :audit_backend) end)
    end
    ...
    test "push_event/1 calls the event store adapter when backend is :eventstore" do
      ...
      Application.put_env(:apm, :event_store_adapter_fn, capture_adapter)
      on_exit(fn -> Application.delete_env(:apm, :event_store_adapter_fn) end)
    ...
    test "adapter returning :ok causes push_event/1 to return :ok" do
      Application.put_env(:apm, :event_store_adapter_fn, fn _, _, _, _ -> :ok end)
      on_exit(fn -> Application.delete_env(:apm, :event_store_adapter_fn) end)
    ...
    test "adapter returning {:error, reason} is propagated as {:error, reason}" do
      Application.put_env(:apm, :event_store_adapter_fn, fn _, _, _, _ ->
