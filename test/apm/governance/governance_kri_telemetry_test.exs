defmodule Apm.Governance.GovernanceKriTelemetryTest do
  @moduledoc """
  ...
  alias Apm.Governance.GovernanceKriPoller
  ...
  test "denial_rate fires when :telemetry.execute is called directly" do
    ref = attach_once([:apm, :governance, :denial_rate])
  ...
  test "escalation_rate fires when :telemetry.execute is called directly" do
    ref = attach_once([:apm, :governance, :escalation_rate])
  ...
  test "critical_command_rate fires when :telemetry.execute is called directly" do
    ref = attach_once([:apm, :governance, :critical_command_rate])
  ...
  test "trust_degradation_events fires when :telemetry.execute is called directly" do
    ref = attach_once([:apm, :governance, :trust_degradation_events])

    :telemetry.execute(
      [:apm, :governance, :trust_degradation_events],
  ...
  test "policy_rule_changes fires on PolicyRulesStore.add_rule/2" do
    ref = attach_once([:apm, :governance, :policy_rule_changes])
  ...
  test "risk_score_p95 fires via GovernanceKriPoller.emit_now/0" do
    ...
    ref = attach_once([:apm, :governance, :risk_score_p95])
