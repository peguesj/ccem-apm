defmodule ApmV5Web.AgentChannelTest do
  ...
  setup do
    ...
    {:ok, _, socket} =
      ApmV5Web.UserSocket
      |> socket(%{})
      |> subscribe_and_join(ApmV5Web.AgentChannel, "agent:fleet")
  ...
  test "joining agent:{id} sends agent_detail" do
    ...
    {:ok, _, _socket} =
      ApmV5Web.UserSocket
      |> socket(%{})
      |> subscribe_and_join(ApmV5Web.AgentChannel, "agent:specific-agent")
  ...
  test "agent:{id} forwards updates for that specific agent" do
    ...
    {:ok, _, _agent_socket} =
      ApmV5Web.UserSocket
      |> socket(%{})
      |> subscribe_and_join(ApmV5Web.AgentChannel, "agent:agent-a")
  ...
  test "send_command replies with error when agent not found" do
    {:ok, _, cmd_socket} =
      ApmV5Web.UserSocket
      |> socket(%{})
      |> subscribe_and_join(ApmV5Web.AgentChannel, "agent:nonexistent")
  ...
  test "send_command replies ok when agent has a path" do
    ...
    {:ok, _, cmd_socket} =
      ApmV5Web.UserSocket
      |> socket(%{})
      |> subscribe_and_join(ApmV5Web.AgentChannel, "agent:cmd-agent")
