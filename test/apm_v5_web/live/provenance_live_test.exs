defmodule ApmV5Web.ProvenanceLiveTest do
  @moduledoc """
  ...
  Run with: mix test test/apm_v5_web/live/provenance_live_test.exs
  """
  ...
  describe "ApmV5Web.ProvenanceLive module" do
    test "module is defined" do
      assert Code.ensure_loaded?(ApmV5Web.ProvenanceLive)
    end

    test "implements LiveView mount/3" do
      assert function_exported?(ApmV5Web.ProvenanceLive, :mount, 3)
    end

    test "implements LiveView render/1" do
      assert function_exported?(ApmV5Web.ProvenanceLive, :render, 1)
    end

    test "defines handle_event/3 for tab switching" do
      # LiveView wraps handle_event/3 — we check the module defines :handle_event
      fns = ApmV5Web.ProvenanceLive.__info__(:functions)
      exported_names = Enum.map(fns, fn {name, _arity} -> name end)

      assert :handle_event in exported_names or
               function_exported?(ApmV5Web.ProvenanceLive, :handle_event, 3),
    ...
    test "defines handle_info/2 for PubSub and tick" do
      ...
      assert :handle_info in exported_names or
               function_exported?(ApmV5Web.ProvenanceLive, :handle_info, 2),
  ...
  describe "router" do
    test "/intelligence/provenance route is registered" do
      routes = Phoenix.Router.routes(ApmV5Web.Router)
      ...
      # e.g. {ApmV5Web.ProvenanceLive, :index, [...], %{...}}
      live_view_module =
      ...
      assert live_view_module == ApmV5Web.ProvenanceLive,
  ...
  describe "sidebar nav" do
    test "intelligence_nav renders Provenance item" do
      ...
      assert ApmV5Web.Components.SidebarNav in source
    end
  end
  ...
  describe "handle_info/2" do
    test "{:artifact_added, _} handler is defined" do
      ...
      assert function_exported?(ApmV5Web.ProvenanceLive, :handle_info, 2)
    end
  ...
  describe "initial assigns" do
    test "tab defaults to :attestations" do
      ...
      result =
        try do
          ApmV5Web.ProvenanceLive.handle_event(
            "switch_tab",
            %{"tab" => "lineage"},
            socket
          )
