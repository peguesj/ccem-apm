defmodule ApmV5.AuditLogTest do
  ...
  setup do
    ...
    _ = Supervisor.terminate_child(ApmV5.Supervisor, AuditLog)
    _ = Supervisor.delete_child(ApmV5.Supervisor, AuditLog)
    ...
    Application.put_env(:apm_v5, :audit_log_dir, @tmp_dir)

    # Start PubSub if not running (test env may not have it)
    unless GenServer.whereis(ApmV5.PubSub) do
      start_supervised!({Phoenix.PubSub, name: ApmV5.PubSub})
    end
    ...
    on_exit(fn ->
      File.rm_rf!(@tmp_dir)
      Application.delete_env(:apm_v5, :audit_log_dir)
    end)
