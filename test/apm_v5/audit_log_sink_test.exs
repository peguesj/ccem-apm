defmodule ApmV5.AuditLogSinkTest do
  @moduledoc """
  Tests for the HTTP audit sink behaviour (audit-s7 / CP-225 / US-457).

  Covers:
  - ApmV5.AuditLog.Sink behaviour contract
  ...
  alias ApmV5.AuditLog.Sinks.NoopSink
  alias ApmV5.AuditLog.Sinks.HttpSink

  # ── NoopSink ─────────────────────────────────────────────────────────────────

  describe "NoopSink" do
    test "implements ApmV5.AuditLog.Sink behaviour" do
      behaviours =
  ...
  describe "HttpSink" do
    ...
    test "push_event/1 fails gracefully when endpoint is unreachable" do
      ...
      Application.put_env(:apm_v5, HttpSink,
      ...
      Application.delete_env(:apm_v5, HttpSink)
    end
  end
  ...
  describe "AuditLog.dispatch_sinks/1" do
    ...
    test "returns :ok with empty sink list" do
      Application.put_env(:apm_v5, :audit_sinks, [])
      assert :ok = AuditLog.dispatch_sinks(%{event_id: "e1"})
      Application.delete_env(:apm_v5, :audit_sinks)
    end

    test "returns :ok with NoopSink configured" do
      Application.put_env(:apm_v5, :audit_sinks, [NoopSink])
    ...
    test "is fire-and-forget: dispatching a sink that sends to test PID completes promptly" do
      ...
      Application.put_env(:apm_v5, :audit_sinks, [ApmV5.AuditLog.Sinks.CaptureSink])
    ...
    test "sink returning {:error, reason} does not crash dispatch" do
      Application.put_env(:apm_v5, :audit_sinks, [ApmV5.AuditLog.Sinks.ErrorSink])
      # Must not raise
...
defmodule ApmV5.AuditLog.Sinks.CaptureSink do
  @moduledoc """
...
defmodule ApmV5.AuditLog.Sinks.ErrorSink do
