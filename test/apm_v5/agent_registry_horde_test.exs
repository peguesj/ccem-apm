defmodule ApmV5.AgentRegistry.HordeTest do
  @moduledoc """
  ...
  alias ApmV5.AgentRegistry.Horde, as: HordeRegistry

  # ---------------------------------------------------------------------------
  # Horde.Registry smoke tests
  # ---------------------------------------------------------------------------

  describe "ApmV5.AgentRegistry.Horde — Horde.Registry sibling" do
    test "HordeRegistry process is alive" do
      pid = Process.whereis(HordeRegistry)
      assert is_pid(pid), "Expected ApmV5.AgentRegistry.Horde to be a running process"
  ...
  describe "ApmV5.AgentRegistry — ETS backend unchanged" do
    setup do
  ...
  describe "configuration — :agent_registry_backend" do
    test "defaults to :ets" do
      backend = Application.get_env(:apm_v5, :agent_registry_backend, :ets)
      assert backend == :ets
    end

    test "config key is readable at runtime" do
      val = Application.get_env(:apm_v5, :agent_registry_backend)
