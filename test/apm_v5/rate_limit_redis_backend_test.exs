defmodule ApmV5.RateLimitRedisBackendTest do
  @moduledoc """
  ...
  Then run:
      ...
      mix test test/apm_v5/rate_limit_redis_backend_test.exs --only rate_limit_redis
  ...
  describe "ETS backend (default, CI-safe)" do
    setup do
      # ApmV5.RateLimit is started by the application supervisor.
      # Hammer uses named ETS tables so we must reuse the running instance.
      # The supervisor starts it under :apm_v5 app — always available in test env.
      :ok
    end

    test "hit/3 returns {:allow, count} on first call" do
      key = "test-ets-#{:erlang.unique_integer([:positive])}"
      assert {:allow, 1} = ApmV5.RateLimit.hit(key, :timer.seconds(60), 100)
    end

    test "hit/3 increments count on repeated calls" do
      key = "test-ets-incr-#{:erlang.unique_integer([:positive])}"
      {:allow, 1} = ApmV5.RateLimit.hit(key, :timer.seconds(60), 100)
      {:allow, 2} = ApmV5.RateLimit.hit(key, :timer.seconds(60), 100)
      {:allow, 3} = ApmV5.RateLimit.hit(key, :timer.seconds(60), 100)
    end

    test "hit/3 returns {:deny, _} when limit exceeded" do
      key = "test-ets-limit-#{:erlang.unique_integer([:positive])}"
      # Set limit to 2
      {:allow, 1} = ApmV5.RateLimit.hit(key, :timer.seconds(60), 2)
      {:allow, 2} = ApmV5.RateLimit.hit(key, :timer.seconds(60), 2)
      result = ApmV5.RateLimit.hit(key, :timer.seconds(60), 2)
    ...
    test "different keys are independent" do
      ...
      {:allow, 1} = ApmV5.RateLimit.hit(key_a, :timer.seconds(60), 5)
      {:allow, 1} = ApmV5.RateLimit.hit(key_b, :timer.seconds(60), 5)
    end
  end
  ...
  describe "ApmV5.RateLimit.EtsBackend" do
    # EtsBackend is started in the supervision tree alongside ApmV5.RateLimit

    test "EtsBackend module is defined" do
      assert Code.ensure_loaded?(ApmV5.RateLimit.EtsBackend)
    end

    test "EtsBackend ETS table exists (supervision tree started it)" do
      # Hammer.ETS creates a named ETS table ApmV5.RateLimit.EtsBackend
      # The backing process is anonymous (no registered name) — verify via ETS
      table_info = :ets.info(ApmV5.RateLimit.EtsBackend)
      assert table_info != :undefined, "Expected ETS table ApmV5.RateLimit.EtsBackend to exist"
    end

    test "EtsBackend.hit/3 works independently of primary module" do
      key = "test-ets-backend-#{:erlang.unique_integer([:positive])}"
      assert {:allow, 1} = ApmV5.RateLimit.EtsBackend.hit(key, :timer.seconds(60), 10)
    end
  end
  ...
  describe "backend configuration" do
    test "configured backend is :ets in test environment" do
      backend = Application.get_env(:apm_v5, ApmV5.RateLimit, []) |> Keyword.get(:backend, :ets)
      assert backend == :ets
    end

    test "ApmV5.RateLimit module is defined" do
      assert Code.ensure_loaded?(ApmV5.RateLimit)
    end

    test "ApmV5.RateLimit responds to hit/3" do
      assert function_exported?(ApmV5.RateLimit, :hit, 3)
    end

    test "ApmV5.RateLimit responds to child_spec/1" do
      spec = ApmV5.RateLimit.child_spec([])
      assert is_map(spec)
  ...
  test "Redis backend hit/3 works when Redis is available" do
    ...
    #   config :apm_v5, ApmV5.RateLimit, backend: :redis
