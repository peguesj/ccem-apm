defmodule ApmV5.Auth.DelegationTokenTest do
  @moduledoc """
  TDD coverage for ApmV5.Auth.DelegationToken (v10.0.0/s2 CP-290 — OWASP MCP02 fix).
  ...
  alias ApmV5.Auth.DelegationToken
  ...
  describe "issue/3 — root tokens (no parent)" do
    test "root token can be issued with any scope", %{keystore: ks} do
      assert {:ok, token} = DelegationToken.issue(nil, "agent-root",
      ...
      assert byte_size(token.signature) == 64  # Ed25519 sig
    end
  end

  describe "issue/3 — child tokens (with parent ceiling)" do
    setup %{keystore: ks} do
      {:ok, parent} = DelegationToken.issue(nil, "parent-1",
        allowed_tools: ["Read", "Write", "Bash", "Edit", "WebFetch"],
    ...
    test "child with subset of parent's tools and lower ceiling — ok", %{parent: parent, keystore: ks} do
      assert {:ok, child} = DelegationToken.issue(parent, "child-ok",
    ...
    test "child with extra tool not in parent — :exceeds_parent_ceiling", %{parent: parent, keystore: ks} do
    ...
    test "child with higher risk ceiling than parent — :exceeds_parent_ceiling", %{parent: parent, keystore: ks} do
    ...
    test "child with same scope as parent — ok (allowed boundary)", %{parent: parent, keystore: ks} do
      assert {:ok, _child} = DelegationToken.issue(parent, "child-equal",
    ...
    test "child of expired parent — :exceeds_parent_ceiling (expired = no scope)", %{keystore: ks} do
      {:ok, dead_parent} = DelegationToken.issue(nil, "dead-parent",
        allowed_tools: ["Read"],
        max_risk_ceiling: :low,
        ttl_seconds: -1,
        keystore: ks
      ...
      assert {:error, reason} = DelegationToken.issue(dead_parent, "ghost-child",
        allowed_tools: ["Read"],
        max_risk_ceiling: :low,
        keystore: ks
      )
  ...
  describe "verify/2" do
    test "valid token verifies", %{keystore: ks} do
      {:ok, token} = DelegationToken.issue(nil, "v-1",
        allowed_tools: ["Read"], max_risk_ceiling: :low, keystore: ks)
      assert :ok = DelegationToken.verify(token, keystore: ks)
    end

    test "expired token rejected", %{keystore: ks} do
      {:ok, token} = DelegationToken.issue(nil, "v-exp",
        allowed_tools: ["Read"], max_risk_ceiling: :low, ttl_seconds: -1, keystore: ks)
      assert {:error, :token_expired} = DelegationToken.verify(token, keystore: ks)
    end

    test "tampered token rejected", %{keystore: ks} do
      {:ok, token} = DelegationToken.issue(nil, "v-t",
        allowed_tools: ["Read"], max_risk_ceiling: :low, keystore: ks)
  ...
  describe "enforce_ceiling/3" do
    setup %{keystore: ks} do
      {:ok, token} = DelegationToken.issue(nil, "enforce-test",
        allowed_tools: ["Read", "Write", "Edit"],
        max_risk_ceiling: :medium,
        keystore: ks)
      {:ok, token: token}
    ...
    test "disallowed tool — :tool_not_allowed", %{token: token} do
      ...
      assert {:error, :tool_not_allowed} = DelegationToken.enforce_ceiling(token, "WebSearch", :none)
    end

    test "allowed tool but risk exceeds ceiling — :risk_exceeds_ceiling", %{token: token} do
      assert {:error, :risk_exceeds_ceiling} = DelegationToken.enforce_ceiling(token, "Read", :high)
      assert {:error, :risk_exceeds_ceiling} = DelegationToken.enforce_ceiling(token, "Write", :critical)
    end

    test "expired token rejected by enforce_ceiling", %{keystore: ks} do
      {:ok, expired} = DelegationToken.issue(nil, "expired",
        allowed_tools: ["Read"], max_risk_ceiling: :low, ttl_seconds: -1, keystore: ks)
