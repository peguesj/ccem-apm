defmodule ApmV5.Plugins.Memory.FederationRouterTest do
  ...
  setup do
    ...
    Application.put_env(:apm_v5, :viki_url, "http://localhost:#{bypass.port}")
    Application.put_env(:apm_v5, :viki_token, "test-token")

    on_exit(fn ->
      Application.delete_env(:apm_v5, :viki_url)
      Application.delete_env(:apm_v5, :viki_token)
    end)
  ...
  describe "MemoryPlugin.handle_action/3 — route_query delegation" do
    test "delegates route_query to FederationRouter", %{bypass: bypass} do
      ...
      assert {:ok, %{results: [], sources_queried: [:viki]}} =
               ApmV5.Plugins.Memory.MemoryPlugin.handle_action(
                 "route_query",
    ...
    test "route_query endpoint is listed in list_endpoints/0" do
      endpoints = ApmV5.Plugins.Memory.MemoryPlugin.list_endpoints()
