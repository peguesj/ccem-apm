defmodule ApmWeb.Plugs.RateLimitHeaders do
  @moduledoc """
  ...
  The plug samples `Apm.RateLimit.hit/3` against the global-ceiling rule
  ...
  Placed **after** `ApmWeb.Plugs.RateLimit` in the `:api` pipeline so
  ...
  defp sample_bucket(ip_key) do
    ...
    case Apm.RateLimit.hit("#{ip_key}:peek", :timer.seconds(@window_seconds), @limit) do
      {:allow, count} ->
