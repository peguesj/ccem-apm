defmodule ApmWeb.V2.ProvenanceController do
  @moduledoc """
  ...
  - `GET /api/v2/provenance/lineage?agent_id=X`
    ...
    Delegates to `Apm.Provenance.LineageTracker` (prov-w2-s6).
  ...
  alias Apm.Provenance.ProvExporter
  alias Apm.Provenance.ArtifactAttestation
  alias Apm.Identity.{KeyStore, DIDProvider, AgentRoleIndex}
  ...
  def lineage(conn, %{"agent_id" => agent_id}) when is_binary(agent_id) do
    tracker = Apm.Provenance.LineageTracker
  ...
  def show(conn, %{"attestation_id" => attestation_id}) do
    alias Apm.Provenance.SLSAProvenance
