defmodule ApmWeb.FormationLive do
        ...
        <div class="ccem-formation-hierarchy flex-1 flex flex-col overflow-hidden">
          ...
          <header class="h-12 bg-base-200 border-b border-base-300 flex items-center justify-between px-4 flex-shrink-0 relative z-10">
            ...
            <div class="flex items-center gap-2">
              <%!-- View mode toggle --%>
              <div
                class="flex gap-0.5 bg-base-300 rounded-lg p-0.5"
                role="tablist"
                aria-label="Formation view mode"
              >
                <button
                  phx-click="set_view_mode"
                  phx-value-mode="graph_td"
                  class={[
                    "btn btn-xs gap-1",
                    if(@view_mode == "graph_td", do: "btn-primary", else: "btn-ghost")
                  ]}
                  role="tab"
                  aria-selected={@view_mode == "graph_td"}
                >
                  <.icon name="hero-arrow-down" class="size-3" /> TD
                </button>
                <button
                  phx-click="set_view_mode"
                  phx-value-mode="graph_lr"
                  class={[
                    "btn btn-xs gap-1",
                    if(@view_mode == "graph_lr", do: "btn-primary", else: "btn-ghost")
                  ]}
                  role="tab"
                  aria-selected={@view_mode == "graph_lr"}
                >
                ...
                <button
                  ...
                  class={[
                    "btn btn-xs gap-1",
                    if(@view_mode == "cards", do: "btn-primary", else: "btn-ghost")
                  ]}
                  role="tab"
                  aria-selected={@view_mode == "cards"}
                >
                ...
                <button
                  ...
                  class={[
                    "btn btn-xs gap-1",
                    if(@view_mode == "tb", do: "btn-primary", else: "btn-ghost")
                  ]}
                  role="tab"
                  aria-selected={@view_mode == "tb"}
                >
                ...
                <button
                  ...
                  class={[
                    "btn btn-xs gap-1",
                    if(@view_mode == "dot", do: "btn-primary", else: "btn-ghost")
                  ]}
                  role="tab"
                  aria-selected={@view_mode == "dot"}
                >
          ...
          <div class="flex-1 flex overflow-hidden">
                ...
                <div class="collapse collapse-arrow bg-base-200 mt-2 shadow-md">
                  ...
                  <div class="collapse-content">
                    <div class="flex flex-wrap gap-4 text-xs">
                      ...
                      <div class="flex items-center gap-2">
                        <svg width="30" height="10">
                          <line x1="0" y1="5" x2="30" y2="5" stroke="#f97316" stroke-width="2.5" />
                        </svg>
                  ...
                  <div class="flex flex-wrap gap-2 items-center">
                    ...
                    <%= for formation <- @formations do %>
                      <button
                        ...
                        class={[
                          "btn btn-xs",
                          if(@dot_formation_id == formation.id,
                            do: "btn-primary",
                            else: "btn-outline"
                          )
                        ]}
                  ...
                  <div :if={@dot_source} class="space-y-2">
                    ...
                    <p :if={@dot_executable} class="text-[10px] text-base-content/30 font-mono">
                      ...
                      <span class="text-base-content/50">
                        dot -Tpng &lt;file.dot&gt; -o formation.png
                      </span>
                    </p>
                  </div>

                  <div
                    :if={is_nil(@dot_source) and @formations != []}
                    class="text-xs text-base-content/30 text-center py-8"
                  >
              ...
              >
                <div class="text-center">
                  ...
                  <p class="text-sm text-base-content/30 mt-1 max-w-md">
                    Formations appear when agents are registered via
                    <code class="font-mono text-xs">/formation deploy</code>
                    or when agents heartbeat with <code class="font-mono text-xs">formation_id</code>
                    metadata.
                  </p>
            ...
            <div class="w-72 border-l border-base-300 bg-base-200 flex flex-col flex-shrink-0 overflow-y-auto">
              ...
              <div
                :if={@wave_progress.total_waves > 0}
                class="px-4 py-3 border-b border-base-300 bg-base-300/30"
              >
                <div class="text-xs text-base-content/50 uppercase tracking-wider mb-1">
                  Wave Progress
                </div>
                <div class="text-sm font-mono text-base-content/80">
                  Wave {@wave_progress.current_wave} of {@wave_progress.total_waves} &mdash; {@wave_progress.agents_complete}/{@wave_progress.agents_in_wave} agents
                </div>
                ...
                <div :if={@selected_node} class="space-y-3">
                  ...
                  <div>
                    <div class={[
                      ...
                      level_badge(@selected_node.level)
                    ]}>
                      {@selected_node.level}
                    </div>
                    <%!-- Agent type badge --%>
                    <span
                      :if={
                        @selected_node[:agent_type] &&
                          @selected_node[:agent_type] not in [nil, "individual"]
                      }
                  ...
                  <div
                    :if={
                      @selected_node[:formation_id] || @selected_node[:squadron] ||
                        @selected_node[:swarm]
                    }
                    class="text-[9px] font-mono leading-loose bg-base-300/40 rounded px-2 py-1.5"
                  >
                    <span :if={@selected_node[:formation_id]} class="text-accent/70">
                      {@selected_node[:formation_id]}
                    </span>
                    <span :if={@selected_node[:squadron]}>
                      <span class="text-base-content/30">›</span>
                      <span class="text-info/70">{@selected_node.squadron}</span>
                    </span>
                    <span :if={@selected_node[:swarm]}>
                      <span class="text-base-content/30">›</span>
                      <span class="text-success/70">{@selected_node.swarm}</span>
                    </span>
                    <span :if={@selected_node[:cluster]}>
                      <span class="text-base-content/30">›</span>
                      <span class="text-secondary/70">{@selected_node.cluster}</span>
                    </span>
                    <span :if={@selected_node.level == "agent"}>
                      <span class="text-base-content/30">›</span>
                      <span class="text-primary/70">{@selected_node.name}</span>
                    </span>
                  </div>
                  ...
                  <div class="space-y-1.5 text-xs">
                    <div :if={@selected_node[:id]} class="flex justify-between gap-2">
                      ...
                      <span class="font-mono text-[9px] text-right truncate">
                        {@selected_node.id}
                      </span>
                    </div>
                    <div :if={@selected_node[:status]} class="flex justify-between">
                      <span class="text-base-content/50">Status</span>
                      <span class={["badge badge-xs", status_badge(@selected_node.status)]}>
                        {@selected_node.status}
                      </span>
                    </div>
                    <div
                      :if={@selected_node[:member_count] && @selected_node[:member_count] > 0}
                    ...
                    <div :if={@selected_node[:story_id]} class="flex justify-between">
                      ...
                      <span class="badge badge-xs badge-primary badge-outline font-mono">
                        {@selected_node.story_id}
                      </span>
                    </div>
                    <div
                      :if={@selected_node[:work_item_title] && !@selected_node[:story_id]}
                      class="flex justify-between gap-2"
                    >
                      <span class="text-base-content/50 flex-shrink-0">Work Item</span>
                      <span class="text-[10px] text-right truncate max-w-[140px]">
                        {@selected_node.work_item_title}
                      </span>
                    </div>
                    <div :if={@selected_node[:plane_issue_id]} class="flex justify-between gap-2">
                      ...
                      <span class="font-mono text-[9px] text-info truncate">
                        {@selected_node.plane_issue_id}
                      </span>
                    </div>
                    ...
                    <div :if={@selected_node[:wave_number]} class="flex justify-between">
                      ...
                      <span class="font-mono text-[10px]">
                        {@selected_node.wave_number}<span
                          :if={@selected_node[:wave_total]}
                          class="text-base-content/40"
                        >/{@selected_node.wave_total}</span>
                      </span>
                    </div>
                    <div
                      :if={!@selected_node[:wave_number] && @selected_node[:wave]}
                      class="flex justify-between"
                    >
                      <span class="text-base-content/50">Wave</span>
                    ...
                    <div :if={@selected_node[:parent_id]} class="flex justify-between gap-2">
                      ...
                      <span class="font-mono text-[9px] text-base-content/50 truncate">
                    ...
                    <div
                      :if={@selected_node[:swarm] && @selected_node.level != "swarm"}
                      class="flex justify-between"
                    >
                    ...
                    <div
                      :if={@selected_node[:cluster] && @selected_node.level != "cluster"}
                      class="flex justify-between"
                    >
  ...
  defp agent_tree_row(assigns) do
    ...
    >
      ...
      <span
        :if={@agent[:agent_type] && @agent[:agent_type] not in [nil, "individual"]}
      ...
      <span :if={@agent[:story_id]} class="font-mono text-[9px] text-primary/70 flex-shrink-0">
        {@agent.story_id}
      </span>
      <span :if={@agent[:wave_number]} class="text-[8px] text-base-content/30 flex-shrink-0">
        W{@agent.wave_number}
      </span>
    </button>
    """
    ...
    <div class="space-y-2 max-w-4xl mx-auto">
      <div class="flex gap-2 mb-4">
        ...
        <select phx-change="filter_role" class="select select-sm select-bordered bg-base-200">
          ...
          <option value="squadron_lead" selected={@role_filter == "squadron_lead"}>
            Squadron Lead
          </option>
          <option value="swarm_agent" selected={@role_filter == "swarm_agent"}>Swarm Agent</option>
          <option value="cluster_agent" selected={@role_filter == "cluster_agent"}>
            Cluster Agent
          </option>
      ...
      <%= for formation <- @formations do %>
        ...
        <details class="border border-base-300 rounded-lg" open>
          <summary class="px-4 py-2 cursor-pointer font-mono text-sm bg-base-300/50 rounded-t-lg flex items-center gap-2 select-none hover:bg-base-300 transition-colors">
            ...
            <span :if={formation[:source] == :upm} class="badge badge-xs badge-outline opacity-50">
              reg
            </span>
          </summary>
          <div class="p-3">
                ...
                <div class="pl-2 border-l-2 border-base-300 space-y-0.5">
                  <%= for agent <- role_agents do %>
                    <button
                      ...
                      phx-value-id={agent.id}
                    >
                      <span class={["w-2 h-2 rounded-full flex-shrink-0", agent_dot(agent.status)]}>
                      </span>
                      <span class="font-mono text-[9px] text-base-content/40 flex-shrink-0 hidden sm:inline">
                        {String.slice(agent.id || "", 0, 16)}
                      </span>
                      <span class="flex-1 truncate">
                        {agent[:work_item_title] || agent[:role] || agent.name}
                      </span>
                      <span :if={agent[:wave_number]} class="badge badge-xs badge-ghost flex-shrink-0">
                        W{agent.wave_number}
                      </span>
                      <span
                        :if={agent[:story_id]}
                        class="font-mono text-[9px] text-primary/70 flex-shrink-0"
                      >
                        {agent.story_id}
                      </span>
                    </button>
        ...
        <div>
          ...
          <div class="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 xl:grid-cols-4 gap-3">
            <%= for agent <- ns_agents do %>
              <button
                class={[
                  "card card-compact bg-base-200 border text-left hover:bg-base-300 transition-colors",
                  cards_agent_border(agent.status)
                ]}
                phx-click="select_agent"
                phx-value-id={agent.id}
              >
                <div class="card-body gap-1.5">
                  <div class="flex items-center justify-between gap-1">
                    <span class={[
                      "badge badge-xs truncate max-w-[100px]",
                      cards_role_badge(agent[:formation_role] || agent[:role])
                    ]}>
                  ...
                  <div
                    :if={agent[:wave_number] || agent[:wave]}
  ...
  def handle_event(
        "select_swarm",
        %{"formation" => fid, "squadron" => sq_name, "swarm" => sw_name},
        socket
      ) do
    formation = Enum.find(socket.assigns.formations, &(&1.id == fid))
  ...
  def handle_event(
        "select_cluster",
        %{"formation" => fid, "squadron" => sq_name, "swarm" => sw_name, "cluster" => cl_name},
        socket
      ) do
    formation = Enum.find(socket.assigns.formations, &(&1.id == fid))
    squadron = if formation, do: Enum.find(formation.squadrons, &(&1.name == sq_name))
  ...
  defp push_formation_graph(socket, formations) do
    ...
    push_event(socket, "formation_data", %{
      nodes: Enum.reverse(nodes),
      edges: Enum.reverse(all_edges)
    })
  end
