      ...
      <:main>
        ...
        <.card style="margin-bottom: 16px; padding: 12px 16px;">
          <div style="display: flex; align-items: center; gap: 16px; flex-wrap: wrap;">
            <div style="display: flex; align-items: center; gap: 8px;">
              <span style="font-size: 11px; color: var(--ccem-fg-muted); text-transform: uppercase; letter-spacing: 0.05em;">
                Status
              </span>
              <.segmented_control
                options={["all", "active", "clashes"]}
            ...
            <div style="display: flex; align-items: center; gap: 8px;">
              ...
              <.segmented_control
                options={["all", "web", "api", "service", "tool"]}
                active={@namespace_filter}
                on_change="namespace_filter"
              />
            </div>
          </div>
        </.card>
        ...
        <div style="display: flex; gap: 16px;">
            ...
            <div style="display: grid; grid-template-columns: repeat(auto-fill, minmax(220px, 1fr)); gap: 12px;">
              <.card :for={p <- @filtered} style="padding: 16px;">
                ...
                <div style="display: flex; align-items: center; gap: 10px; margin-bottom: 12px;">
                  <span style="font-size: 24px; font-family: monospace; font-weight: 700; color: var(--ccem-fg);">
                    :{p.port}
                  </span>
                  <.badge tone={if p.active, do: "success", else: "neutral"} dot={true}>
                    {if p.active, do: "active", else: "inactive"}
                  </.badge>
                </div>
                <div
                  :if={MapSet.member?(@clash_ports, p.name)}
                  style="padding: 8px; border-radius: 6px; background: var(--ccem-err-bg, rgba(239,68,68,0.08)); border: 1px solid var(--ccem-err-border, rgba(239,68,68,0.2)); display: flex; align-items: center; justify-content: space-between;"
                >
                  <.badge tone="error">Port clash</.badge>
                  <.btn
                    variant="destructive"
                    size="xs"
                    phx-click="assign_port"
                    phx-value-project={p.name}
                  >
                    Reassign
                  </.btn>
                </div>
          ...
          <div style="width: 240px; flex-shrink: 0;">
            <.card style="padding: 16px;">
              <h3 style="font-size: 12px; font-weight: 600; color: var(--ccem-fg-muted); margin: 0 0 16px 0;">
                Port Ranges
              </h3>
              <div style="display: flex; flex-direction: column; gap: 12px;">
                <div
                  :for={{ns, range} <- @port_ranges}
                  style="display: flex; flex-direction: column; gap: 4px;"
                >
                  <div style="display: flex; justify-content: space-between; font-size: 11px;">
                    <span style="color: var(--ccem-fg-muted); text-transform: capitalize;">{ns}</span>
                    <span style="color: var(--ccem-fg-subtle); font-family: monospace;">
                      {range.first}-{range.last}
                    </span>
                  </div>
                  <div style="height: 6px; background: var(--ccem-surface-2); border-radius: 3px; overflow: hidden;">
                    <div style={"height: 100%; border-radius: 3px; background: var(--ccem-accent); width: #{min(trunc(Range.size(range) / 70 * 100), 100)}%;"}>
                    </div>
                  </div>
                </div>
              </div>
        ...
        <div :if={@clash_count > 0} style="margin-top: 16px;">
          <.card style="border-color: var(--ccem-err-border, rgba(239,68,68,0.3)); padding: 16px;">
            <h3 style="font-size: 13px; font-weight: 600; color: var(--ccem-err, #ef4444); margin: 0 0 12px 0;">
              Clash Resolution
            </h3>
            <div style="display: flex; flex-direction: column; gap: 8px;">
              <div
                :for={clash <- @clashes}
                style="display: flex; align-items: center; justify-content: space-between; padding: 10px 12px; background: var(--ccem-surface-1); border-radius: 6px; border: 1px solid var(--ccem-border);"
              >
                <div style="font-size: 13px; display: flex; align-items: center; gap: 8px;">
                  <span style="color: var(--ccem-fg-muted);">Port</span>
                  <span style="font-family: monospace; font-weight: 700; color: var(--ccem-fg);">
                    {clash.port}
                  </span>
