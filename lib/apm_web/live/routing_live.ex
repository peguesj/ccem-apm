defmodule ApmWeb.RoutingLive do
  @moduledoc """
  ...
  alias Apm.Auth.{AuthorizationGate, SessionStore, ContextTracker}
  ...
  def handle_event("approve_gate", %{"gate_id" => gate_id}, socket) do
    try do
      Apm.AgUi.ApprovalGate.approve(gate_id)
    rescue
      _ -> :ok
    end
  ...
  def handle_event("reject_gate", %{"gate_id" => gate_id}, socket) do
    try do
      Apm.AgUi.ApprovalGate.reject(gate_id, "Rejected via routing graph")
    rescue
      _ -> :ok
    end
  ...
  defp build_routing_graph do
    ...
    pending_gates =
      try do
        Apm.AgUi.ApprovalGate.list_pending()
      rescue
        _ -> []
      catch
        :exit, _ -> []
