defmodule ApmWeb.ConversationMonitorLive do
  @moduledoc """
  ...
  defp load_conversations do
    Apm.ConversationWatcher.get_conversations()
  end

  @spec load_messages(String.t()) :: {list(map()), non_neg_integer()}
  defp load_messages(path) do
    case Apm.ConversationReader.read_recent(path, 100) do
      {:ok, messages} ->
        offset =
          case Apm.ConversationReader.file_size(path) do
            {:ok, size} -> size
            _ -> 0
          end
  ...
  defp poll_live_messages(socket) do
    ...
    case Apm.ConversationReader.read_from_offset(path, offset) do
