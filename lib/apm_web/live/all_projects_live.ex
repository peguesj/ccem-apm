defmodule ApmWeb.AllProjectsLive do
  @moduledoc """
                  ...
                  <div class="card bg-base-200 border border-base-300 shadow-xl">
                    <div class="card-body p-3">
                      ...
                      <div class="space-y-1 max-h-48 overflow-y-auto">
                        <div
                          :for={n <- Enum.take(@notifications, 8)}
                        ...
                        <p
                          :if={@notifications == []}
                          class="text-center text-base-content/40 py-3 text-xs"
                        >
                          No notifications
                        </p>
                      </div>
            ...
            <div class="grid grid-cols-12 gap-4 auto-rows-min">
              ...
              >
                <div class="grid grid-cols-2 sm:grid-cols-3 lg:grid-cols-6 gap-3">
                  ...
                  <.kpi
                    label="Active"
                    value={Enum.count(@agents, &(&1.status == "active"))}
                    color="text-success"
                    icon="hero-play"
                  />
                  <.kpi
                  ...
                  <.kpi
                    label="Errors"
                    value={Enum.count(@agents, &(&1.status == "error"))}
                    color="text-error"
                    icon="hero-exclamation-triangle"
                  />
                  <.kpi
                    label="Notifications"
                    value={length(@notifications)}
                    color="text-warning"
                    icon="hero-bell"
                  />
                </div>
                ...
                <div class="space-y-2 overflow-y-auto" style={widget_height(@widgets["projects"])}>
                  <div
                    ...
                    class={[
                      ...
                      (@drill_project == project["name"] && "border-primary bg-primary/5") ||
                  ...
                  >
                    ...
                    <div class="text-[10px] text-base-content/30 mt-1 truncate">
                      {project["root"]}
                    </div>
                  </div>
              ...
              >
                <div
                  ...
                  style={widget_height(@widgets["notifications"])}
                >
                  <div
                  ...
                  >
                    <div class="flex items-center gap-2">
                      <span class={["badge badge-xs flex-shrink-0", notif_class(n.level)]}>
                        {n.level}
                      </span>
                      <span class="font-medium truncate flex-1">{n.title}</span>
                      <span class="text-base-content/30 text-[10px] flex-shrink-0">
                        {format_notif_time(n)}
                      </span>
                    </div>
              ...
              >
                <div class="space-y-3 overflow-y-auto" style={widget_height(@widgets["ralph-status"])}>
                  <div
                    ...
                    class={[
                      "rounded border p-3",
                      (project["prd_json"] != "" && "border-base-300") ||
                        "border-base-300/40 opacity-50"
                    ]}
                  >
              ...
              >
                <div class="space-y-1.5 overflow-y-auto" style={widget_height(@widgets["activity"])}>
                  <div
                    :for={session <- recent_sessions(@config)}
                    class="flex items-start gap-3 p-2 rounded bg-base-300 text-xs hover:bg-base-100 transition-colors"
                  >
                    <div class="w-2 h-2 rounded-full bg-success mt-1 flex-shrink-0 animate-pulse">
                    </div>
                ...
                <div class="overflow-y-auto" style="max-height: 220px;">
                  ...
                  <%= if @inspector_tab == :ports do %>
                    <div class="flex flex-col items-center justify-center py-8 gap-2 text-base-content/30">
                      ...
                      <.link navigate={~p"/"} class="btn btn-xs btn-ghost text-primary mt-1">
                        Go to Dashboard
                      </.link>
                    </div>
                  <% end %>
        ...
        <div class="flex items-center gap-1">
          <button
            class={[
              ...
              (@widget.locked && "text-warning") || "text-base-content/30"
            ]}
          ...
          >
            <.icon
              name={if @widget.locked, do: "hero-lock-closed", else: "hero-lock-open"}
              class="size-3"
            />
          </button>
          <button
          ...
          >
            <.icon
              name={if @widget.collapsed, do: "hero-chevron-down", else: "hero-chevron-up"}
              class="size-3"
            />
          </button>
        </div>
  ...
  def handle_event("toggle_lock", %{"id" => widget_id}, socket) do
    widgets =
      update_widget(socket.assigns.widgets, widget_id, fn w ->
        %{w | locked: !w.locked}
      end)
  ...
  defp uptime, do: Apm.Uptime.formatted_short()
