defmodule ApmWeb.WorkflowLive do
  @moduledoc """
  ...
  def mount(%{"type" => "upm"}, _session, socket) do
    if connected?(socket) do
      Phoenix.PubSub.subscribe(Apm.PubSub, "upm:status")
    end

    case (try do
            WorkflowRegistry.get_workflow("upm")
          rescue
            _ -> nil
          catch
            :exit, _ -> nil
  ...
  def mount(%{"type" => type}, _session, socket) do
    case (try do
            WorkflowRegistry.get_workflow(type)
          rescue
            _ -> nil
          catch
            :exit, _ -> nil
    ...
    stories =
      case assigns do
        ...
        %{current_context: %{active_story: _}} ->
          ...
          case Apm.UpmStore.get_status() do
          ...
          <header class="h-12 bg-base-200 border-b border-base-300 flex items-center justify-between px-4 flex-shrink-0 relative z-10">
            ...
            <div class="flex items-center gap-2">
              <%= if @active_tab == "current" do %>
                ...
                >
                  <.icon name="hero-bars-3-bottom-right" class="w-4 h-4" /> Inspector
                </button>
              <% end %>
            </div>
          </header>
  ...
  defp render_current_tab(assigns) do
    ...
    <div class="flex flex-1 overflow-hidden">
      ...
      <div class="flex-1 overflow-y-auto p-6 space-y-4">
        ...
        <div class="bg-base-200 rounded-xl p-5">
          <h2 class="text-xs font-semibold text-base-content/50 uppercase tracking-wider mb-4">
            UPM Status
          </h2>
    ...
    <!-- TSC Gate -->
        <div class="bg-base-200 rounded-xl p-5">
          ...
          <div class="flex items-center gap-3">
            ...
            <code class="text-sm font-mono bg-base-300 px-3 py-1 rounded-lg">
              {@context.tsc_gate}
            </code>
          </div>
        </div>
            ...
            <div class="space-y-2">
              ...
              <div class="flex items-center justify-between">
                ...
                <span class="badge badge-primary badge-sm">
                  {Map.get(@context.formation, :status, "unknown")}
                </span>
              </div>
              <div class="flex items-center justify-between">
    ...
    <!-- Stories Plan View -->
        <%= if @stories != [] do %>
          <div class="bg-base-200 rounded-xl p-5">
            <h2 class="text-xs font-semibold text-base-content/50 uppercase tracking-wider mb-3">
              Stories <span class="ml-2 badge badge-ghost badge-xs">{length(@stories)}</span>
            </h2>
            <div class="space-y-1.5">
              <%= for story <- @stories do %>
                ...
                >
                  ...
                  <.icon
                    ...
                    class="w-3.5 h-3.5 text-base-content/30 flex-shrink-0"
                  />
                </button>
              <% end %>
            </div>
    ...
    <!-- Plane PM link -->
              <%= if Map.get(@selected_story, :plane_issue_id) do %>
                <div>
                  ...
                  >
                    <.icon name="hero-arrow-top-right-on-square" class="w-3 h-3" /> View in Plane PM
                  </a>
                </div>
              <% end %>
            </div>
