defmodule ApmWeb.AuthorizationLive do
  @moduledoc """
  ...
  alias Apm.Auth.{AuthorizationGate, PendingDecisions, PolicyRulesStore, SessionStore}
  ...
  def mount(_params, _session, socket) do
    if connected?(socket) do
      ...
      Phoenix.PubSub.subscribe(Apm.PubSub, "agentlock:sessions")
      Phoenix.PubSub.subscribe(Apm.PubSub, "agentlock:trust")
      Phoenix.PubSub.subscribe(Apm.PubSub, "apm:agentlock")
      Phoenix.PubSub.subscribe(Apm.PubSub, "agentlock:pending")
      Phoenix.PubSub.subscribe(Apm.PubSub, @pubsub_sessions_topic)
  ...
  def handle_event("approve_for", %{"id" => request_id, "minutes" => minutes_str}, socket) do
    ...
    if entry do
      ...
      Apm.Auth.AutoApprovalStore.create(%{
  ...
  defp load_recent_decisions do
    try do
      Apm.AuditLog.tail(@max_decisions)
      |> Enum.filter(fn e ->
        et = Map.get(e, :event_type, "")
    ...
    try do
      ...
      case File.read(config_path) do
        {:ok, content} ->
          ...
          Phoenix.PubSub.broadcast(Apm.PubSub, "apm:settings", {:settings_updated, settings})
