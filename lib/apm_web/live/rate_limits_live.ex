defmodule ApmWeb.RateLimitsLive do
  @moduledoc """
  Standalone LiveView page at `/rate-limits`.

  Hosts `ApmWeb.Components.RateLimitWidget` and subscribes to the
  ...
  alias Apm.Auth.AdaptiveRateLimiter

  @impl true
  def mount(_params, _session, socket) do
    if connected?(socket) do
      Phoenix.PubSub.subscribe(Apm.PubSub, "apm:rate_limits")
  ...
  def handle_info({:adaptive_scaled, factor}, socket) do
    send_update(ApmWeb.Components.RateLimitWidget,
  ...
  def render(assigns) do
    ...
    <div class="flex-1 overflow-y-auto p-6">
      <div class="max-w-3xl mx-auto">
        ...
        <.live_component
          module={ApmWeb.Components.RateLimitWidget}
          id="rate-limit-widget"
        />
      </div>
    </div>
