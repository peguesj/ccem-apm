defmodule ApmWeb.ArchitectureLive do
  @moduledoc """
  ...
  ## Data
  Reads from `Apm.Architectures.ArchitectureStore` and `Apm.AgentRegistry`.
  No PubSub subscription (static architecture data). Refreshes every 10 seconds.
  """

  use ApmWeb, :live_view

  alias Apm.Architectures.ArchitectureStore
  ...
  def mount(_params, _session, socket) do
    if connected?(socket) do
      Phoenix.PubSub.subscribe(Apm.PubSub, "apm:architectures")
  ...
  defp load_pubsub_topics do
    ...
    Enum.map(known_topics, fn topic ->
      subscriber_count =
        try do
          Phoenix.PubSub.node_name(Apm.PubSub)
