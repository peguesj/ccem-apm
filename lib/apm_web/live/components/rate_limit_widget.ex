defmodule ApmWeb.Components.RateLimitWidget do
  @moduledoc """
  ...
  `ApmWeb.RateLimitsLive` which handles the subscription itself.

  ## TODO

  - Top-10 rate-limited agents: requires an ETS enumeration helper in
    `Apm.RateLimit` to expose per-key hit counts.  Tracked in rl-s9.
  ...
  alias Apm.Auth.{AdaptiveRateLimiter, FormationRateLimiter}
