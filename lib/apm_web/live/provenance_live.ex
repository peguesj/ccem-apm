defmodule ApmWeb.ProvenanceLive do
  ...
  def handle_event("refresh_attestations", _params, socket) do
    ...
    {:noreply,
     assign(socket, attestations: attestations, attestation_count: length(attestations))}
  end
                      ...
                      <%= for attest <- @attestations do %>
                        <tr style="border-bottom:1px solid var(--ccem-border);hover:background:var(--ccem-surface-hover);">
                          ...
                          <td style="padding:0.5rem 0.75rem;">
                            <.badge tone="neutral" style="font-size:0.7rem;">
                              {attest.tool_name || "—"}
                            </.badge>
                          </td>
                          <td style="padding:0.5rem 0.75rem;font-family:var(--ccem-font-mono);font-size:0.75rem;color:var(--ccem-text-muted);">
  ...
  defp file_path(%ArtifactAttestation{subject: [h | _]}) when is_map(h),
    do: Map.get(h, :name) || Map.get(h, "name") || "—"
  ...
  defp format_timestamp(ts) when is_binary(ts),
    do: String.slice(ts, 0, 19) |> String.replace("T", " ")

  defp format_timestamp(_), do: "—"

  defp sig_valid?(%ArtifactAttestation{signature: sig} = attest)
