defmodule ApmWeb.PluginDashboardLive do
  @moduledoc """
  ...
  import ApmWeb.Components.GettingStartedWizard
  ...
  def mount(_params, _session, socket) do
    if connected?(socket) do
      ...
      Phoenix.PubSub.subscribe(Apm.PubSub, @integrations_topic)
      Phoenix.PubSub.subscribe(Apm.PubSub, "apm:cc_plugins")
      Phoenix.PubSub.subscribe(Apm.PubSub, "apm:plugin_repos")
    end
      ...
      {:error, :not_found} ->
        {:noreply,
         ...
         |> assign(
           active_tab: "mcp",
           current_path: "/plugins/#{slug}",
           page_title: "Plugin: #{humanize(slug)}"
         )
  ...
  def handle_params(
        %{"slug" => slug},
        _uri,
        %{assigns: %{live_action: :integration_show}} = socket
      ) do
    {:noreply,
     socket
     |> assign(
       active_tab: "integrations",
       current_path: "/integrations/#{slug}",
       page_title: "Integration: #{humanize(slug)}"
     )
  ...
  def handle_event("run_plugin_action", _params, socket) do
    ...
    if is_nil(plugin) or is_nil(action) do
      ...
      Task.start(fn ->
        result = Apm.Plugins.PluginRegistry.call_plugin_action(plugin.name, action, params)
  ...
  def handle_event("detail_run_action", %{"plugin" => plugin_name, "action" => action}, socket) do
    ...
    Task.start(fn ->
      result = Apm.Plugins.PluginRegistry.call_plugin_action(plugin_name, action, %{})
  ...
  def handle_event("run_integration_event", _params, socket) do
    ...
    if is_nil(integration) or is_nil(event) do
      {:noreply,
       assign(socket, integration_event_error: "Select an integration and event first.")}
    else
      params =
        case Jason.decode(raw) do
          {:ok, map} -> map
      ...
      socket =
        assign(socket,
          integration_event_running: true,
          integration_event_error: nil,
          integration_event_result: nil
        )
      ...
      Task.start(fn ->
        result = Apm.Integrations.IntegrationRegistry.call_integration_event(name, event, params)
  ...
  def handle_event("reload_integrations", _params, socket) do
    Apm.Integrations.IntegrationRegistry.reload_defaults()
    Process.sleep(200)
  ...
  def handle_event("rescan_cc", _params, socket) do
    Apm.Plugins.ClaudeCodePluginBridge.rescan()
    Process.sleep(300)
  ...
  def handle_event("add_repo", %{"name" => name, "url" => url}, socket) do
    Apm.Plugins.PluginRepositoryStore.add_repo(%{
      "name" => name,
      "url" => url,
      "source" => "custom"
    })
  ...
  def handle_event("delete_repo", %{"name" => name}, socket) do
    Apm.Plugins.PluginRepositoryStore.delete_repo(name)
    {:noreply, assign_repos(socket)}
  end
  ...
  defp assign_data(socket) do
    socket
    |> assign(
      mcp_servers: Apm.PluginScanner.get_mcp_servers(),
      plugins: Apm.PluginScanner.get_plugins(),
  ...
  defp assign_registered_plugins(socket) do
    assign(socket, registered_plugins: Apm.Plugins.PluginRegistry.list_plugins())
  end
  ...
  defp assign_cc_plugins(socket) do
    plugins = Apm.Plugins.ClaudeCodePluginBridge.list_cc_plugins()
    summary = Apm.Plugins.ClaudeCodePluginBridge.get_summary()
    assign(socket, cc_plugins: plugins, cc_plugins_summary: summary)
  end

  defp assign_repos(socket) do
    assign(socket, plugin_repos: Apm.Plugins.PluginRepositoryStore.list_repos())
  end
  ...
  defp load_plane_issues(socket) do
    case Apm.Plugins.PluginRegistry.call_plugin_action("plane", "board_state", %{}) do
      ...
      {:error, {:not_found, _}} ->
        assign(socket,
          ...
          plane_issues: [],
          plane_total: 0,
          plane_error: "Plane plugin not registered"
        )
    ...
    <.page_layout sidebar_collapsed={@sidebar_collapsed} inspector_open={@inspector_open}>
      ...
      <:main>
        <div class="flex-1 flex flex-col overflow-hidden min-w-0">
          ...
          <div
            :if={@active_tab != "plugin_detail"}
            class="border-b border-base-300 bg-base-200 px-4 flex gap-1 flex-shrink-0 overflow-x-auto"
          >
            <button
              :if={@current_path != "/integrations"}
            ...
            <button
              :if={@current_path != "/integrations"}
              phx-click="switch_tab"
              phx-value-tab="claude_code"
              class={"tab tab-bordered tab-sm #{if @active_tab == "claude_code", do: "tab-active", else: ""}"}
            >
              Claude Code
              <span
                :if={Map.get(@cc_plugins_summary, :total_installed, 0) > 0}
          ...
          <div class="flex-1 overflow-hidden flex">
            <div class="flex-1 overflow-y-auto p-4 space-y-4">
              ...
              <div :if={@active_tab == "plugin_detail" and @detail_plugin != nil}>
                <.plugin_detail_view
                  plugin={@detail_plugin}
                  selected_plugin_slug={@selected_plugin_slug}
                  selected_plugin={@selected_plugin}
                  selected_plugin_action={@selected_plugin_action}
                  plugin_action_params={@plugin_action_params}
                  plugin_action_result={@plugin_action_result}
                  plugin_action_error={@plugin_action_error}
                  plugin_action_running={@plugin_action_running}
                />
              </div>
              ...
              <div :if={@active_tab == "registered"}>
                <div :if={@registered_plugins == []} class="bg-base-200 rounded-lg p-4">
                  <.empty_state
                    ...
                    hint="Engine plugins self-register with Apm.Plugins.PluginRegistry on boot. If the list is empty, the registry may not have started."
                  />
                </div>
                    ...
                    <div class="card-body">
                      <div class="flex items-center justify-between">
                        ...
                        <div class="flex gap-1 items-center">
                          <span :if={plugin.scope == :apm} class="badge badge-xs badge-accent">
                            APM
                          </span>
              ...
              <div :if={@active_tab == "integrations"}>
                <div
                  :if={@integrations == []}
                  class="bg-base-200 rounded-lg p-4 text-xs text-base-content/40 text-center py-8"
                >
              ...
              <div :if={@active_tab == "plane"} class="space-y-4">
                ...
                <div
                  :if={@plane_board == [] and is_nil(@plane_error)}
                  class="text-xs text-base-content/40 py-8 text-center"
                >
                      ...
                      >
                        <div class="flex items-start justify-between gap-1">
                          ...
                          <span
                            ...
                            class={"badge badge-xs #{priority_class(issue.priority)}"}
                          >
                            {issue.priority}
                          </span>
                        </div>
                ...
                <div class="grid grid-cols-3 gap-3">
                  ...
                  <div class="bg-base-200 rounded-lg p-3 text-center">
                    <p class="text-xl font-bold text-success">
                      {Map.get(@cc_plugins_summary, :enabled_count, 0)}
                    </p>
                    <p class="text-xs text-base-content/50">Enabled</p>
                  </div>
                  <div class="bg-base-200 rounded-lg p-3 text-center">
                    <p class="text-xl font-bold text-info">
                      {Map.get(@cc_plugins_summary, :marketplace_count, 0)}
                    </p>
                  ...
                  >
                    <div class="card-body">
                      ...
                      <div class="flex flex-wrap gap-1 mt-1">
                        ...
                        <span :if={plugin.scope} class="badge badge-xs badge-outline">
                          {plugin.scope}
                        </span>
                      </div>
                      <p
                        :if={plugin.description}
                        class="text-xs text-base-content/60 line-clamp-2 mt-1"
                      >
                        {plugin.description}
                      </p>
                      <div class="flex items-center justify-between mt-2">
                        ...
                        <span
                          :if={is_list(plugin.skills) and length(plugin.skills) > 0}
            ...
            <div
              :if={@active_tab == "registered" && @selected_plugin}
              class="w-80 xl:w-96 border-l border-base-300 bg-base-100 flex flex-col flex-shrink-0 overflow-hidden"
            >
              <div class="flex items-center justify-between p-3 border-b border-base-300">
                <div>
                  <div class="flex items-center gap-2">
                    <p class="font-semibold text-sm">{@selected_plugin.name}</p>
                    <span :if={@selected_plugin.scope == :apm} class="badge badge-xs badge-accent">
                      APM
                    </span>
                  </div>
            ...
            <div
              :if={@active_tab == "integrations" && @selected_integration}
              class="w-80 xl:w-96 border-l border-base-300 bg-base-100 flex flex-col flex-shrink-0 overflow-hidden"
            >
              <div class="flex items-center justify-between p-3 border-b border-base-300">
                <div>
        ...
        >
          ...
          <div class="flex-1 overflow-y-auto p-4 space-y-4">
            ...
            <div class="flex flex-wrap gap-2">
              ...
              <span
                :if={@selected_issue.priority && @selected_issue.priority != "none"}
  ...
  defp plugin_detail_view(assigns) do
    ...
    <div class="max-w-4xl mx-auto space-y-6">
      ...
      <div :if={(@plugin.endpoints || []) != []} class="bg-base-200 rounded-lg p-4">
          ...
          <%= for ep <- @plugin.endpoints do %>
            <div class="flex items-center justify-between bg-base-300/50 rounded px-3 py-2">
              <div class="flex-1">
                ...
                <span :if={ep[:description]} class="text-xs text-base-content/40 ml-2">
                  {ep.description}
                </span>
              </div>
              <button
        ...
        <div
          :if={@plugin_action_result}
          class="mt-4 bg-success/10 border border-success/30 rounded-lg p-3"
        >
      ...
      <div :if={(@plugin[:integration_modules] || []) != []} class="bg-base-200 rounded-lg p-4">
        ...
        <div class="flex flex-wrap gap-2">
          <span
            :for={int_mod <- @plugin.integration_modules}
      ...
      <div :if={(@plugin.endpoints || []) == []} class="bg-base-200 rounded-lg p-8 text-center">
        ...
        <p class="text-xs text-base-content/30 mt-1">
          It may provide widgets, integrations, or background services.
        </p>
      </div>
    </div>
    """
