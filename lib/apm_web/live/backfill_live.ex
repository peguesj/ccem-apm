defmodule ApmWeb.BackfillLive do
  @moduledoc """
  ...
  def handle_event("sync_to_plane", _params, socket) do
    ...
    Task.start(fn ->
      Apm.BackfillRunner.run_primary()
    end)
  ...
  def handle_event("check_api", _params, socket) do
    result = Apm.BackfillRunner.check_api_connection()
  ...
  def handle_event("insert_rule", _params, socket) do
    result = Apm.UpmPersistentRule.insert_rule()
  ...
  defp assign_data(socket) do
    store = Apm.BackfillStore.get_state()
    prd_files = Apm.PrdScanner.scan()
    rule_status = Apm.UpmPersistentRule.check_rule()
        ...
        <div class="flex-1 flex flex-col overflow-hidden">
          <header class="h-12 bg-base-200 border-b border-base-300 flex items-center justify-between px-4 flex-shrink-0 relative z-10">
            <div class="flex items-center gap-3">
              <h2 class="text-sm font-semibold text-base-content">UPM / Plane Backfill</h2>
            </div>
            <div class="flex items-center gap-2">
              ...
              <button
                phx-click="sync_to_plane"
                disabled={@syncing}
                class="btn btn-xs btn-primary gap-1"
              >
                <.icon
                  name={if @syncing, do: "hero-arrow-path", else: "hero-arrow-up-tray"}
          ...
          <div class="flex-1 overflow-y-auto p-4 space-y-4">
            ...
            <div class="bg-base-200 rounded-lg p-4">
              ...
              <div :if={match?({:absent, _}, @rule_status)} class="flex items-center gap-2">
                <span class="text-xs text-base-content/60">
                  Rule not found in ~/.claude/CLAUDE.md
                </span>
            ...
            <div class="bg-base-200 rounded-lg p-4">
              ...
              <div :if={@prd_files == []} class="text-xs text-base-content/40">
                No prd.json files found
              </div>
            ...
            <div class="bg-base-200 rounded-lg p-4">
              ...
              <div :if={@store.runs == []} class="text-xs text-base-content/40">
                No runs yet. Click "Sync to Plane" to start.
              </div>
              <div class="space-y-2">
                <div
                  :for={run <- Enum.take(@store.runs, 10)}
                  class={["rounded p-2 text-xs border-l-4", run_border_class(run)]}
                >
                  ...
                  <div :if={run[:synced]} class="mt-1 text-base-content/60">
                    {Enum.count(run[:synced], &(&1.status == :synced))} synced / {Enum.count(
                      run[:synced],
                      &(&1.status == :error)
                    )} errors
                  </div>
