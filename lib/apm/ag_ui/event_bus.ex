defmodule Apm.AgUi.EventBus do
  @moduledoc """
  ...
  ## Topics
  Events are published to hierarchical topics defined by Apm.AgUi.Topics.
  ...
  def subscribe(pattern, opts \\ []) do
    if opts[:persistent] do
      Apm.AgUi.SubscriptionStore.register(pattern, inspect(self()))
    end
  ...
  def init(_opts) do
    ...
    Apm.AgUi.SubscriptionStore.init()
    Apm.AgUi.SubscriptionStore.restore()
  ...
  def handle_cast({:dispatch, type, event}, state) do
    topic = Apm.AgUi.Topics.topic_for(type)
    ...
    if since_seq > 0 and since_seq < oldest_seq do
      ...
      filtered =
        ...
        |> Enum.filter(fn {seq, topic, _event} ->
          seq > since_seq and
            (is_nil(topic_filter) or Apm.AgUi.Topics.matches?(topic_filter, topic))
        end)
  ...
  defp deliver_to_subscribers(topic, event) do
    ...
    |> Enum.each(fn {pattern, pid, _ref} ->
      if Apm.AgUi.Topics.matches?(pattern, topic) do
