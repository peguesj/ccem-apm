defmodule Apm.AgUi.EventRouter do
  @moduledoc """
  ...
  defp route_to_agent_registry(event_type, data) do
    ...
    if agent_id do
      case event_type do
        :run_started ->
          Apm.AgentRegistry.register_agent(agent_id, %{
            status: "active",
  ...
  defp route_to_formation(data) do
    ...
    if formation_id do
      ...
      try do
        Apm.UpmStore.record_event(%{"formation_id" => formation_id, "data" => data})
      rescue
        _ -> :ok
      end
    end
  ...
  defp route_to_metrics(event_type, data) do
    try do
      Apm.MetricsCollector.record(data[:agent_id] || "system", to_string(event_type), 1)
    rescue
      _ -> :ok
    end
  end
