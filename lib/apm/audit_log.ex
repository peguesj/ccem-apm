defmodule Apm.AuditLog do
  ...
  ## Example
      ...
      iex> {page1, cursor} = Apm.AuditLog.query_page(limit: 10)
      iex> {page2, _cursor2} = Apm.AuditLog.query_page(after: cursor, limit: 10)
  """
  ...
  Configurable via `config :apm, :audit_log_retention_online_days, N`.
  Default: 90 days.
  """
  @spec online_days() :: pos_integer()
  def online_days, do: Application.get_env(:apm, :audit_log_retention_online_days, 90)
  ...
  Configurable via `config :apm, :audit_log_retention_archive_days, N`.
  Default: 365 days.
  """
  @spec archive_days() :: pos_integer()
  def archive_days, do: Application.get_env(:apm, :audit_log_retention_archive_days, 365)
  ...
  defp do_log(event_type, actor, resource, details, correlation_id, context, state)
    ...
    stored_details =
      if is_map(details) and Apm.Governance.Vault.sensitive?(details) do
        Apm.Governance.Vault.encrypt_details(details)
      else
        details
      end
    ...
    Apm.Provenance.ArtifactAttestation.Signer.maybe_attest(
      event_type,
      resource,
      agent_id,
      tool_name,
  ...
  Sinks are resolved at call-time from `config :apm, :audit_sinks`.  The
  default is `[]` (no sinks).  Each sink module MUST implement the
  `Apm.AuditLog.Sink` behaviour.
  ...
  def dispatch_sinks(event) do
    sinks = Application.get_env(:apm, :audit_sinks, [])
  ...
  defp log_dir do
    Application.get_env(:apm, :audit_log_dir, @log_dir)
  end
  ...
  defp maybe_decrypt(events, true) do
    Enum.map(events, fn event ->
      if is_map(event.details) do
        Map.put(event, :details, Apm.Governance.Vault.decrypt_details(event.details))
      else
        event
      end
    end)
