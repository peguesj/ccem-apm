defmodule Apm.Telemetry.BootReporter do
  @moduledoc """
  ...
  def init(_opts) do
    ...
    try do
      Phoenix.PubSub.subscribe(Apm.PubSub, "apm:boot")
    rescue
      _ -> :ok
    catch
      :exit, _ -> :ok
  ...
  defp emit_notification(title, message, severity) do
    ...
    try do
      Phoenix.PubSub.broadcast(Apm.PubSub, "apm:notifications", {:boot_event, notif})
    rescue
      _ -> :ok
    catch
      :exit, _ -> :ok
