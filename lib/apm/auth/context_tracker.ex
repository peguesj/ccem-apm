defmodule Apm.Auth.ContextTracker do
  @moduledoc """
  ...
  alias Apm.Auth.Types.ContextEntry
  ...
  def handle_call({:record_write, session_id, agent_id, source, content_hash}, _from, state) do
    ...
    if new_ceiling != session_state.ceiling do
      ...
      try do
        Apm.Auth.SessionStore.update_trust(session_id, new_ceiling)
      rescue
        _ -> :ok
      end
    end
  ...
  defp broadcast(event) do
    Phoenix.PubSub.broadcast(Apm.PubSub, "agentlock:trust", event)
  end
end
