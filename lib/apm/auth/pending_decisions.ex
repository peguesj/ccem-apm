defmodule Apm.Auth.PendingDecisions do
  @moduledoc """
  ...
  alias Apm.Auth.TokenStore
  alias Apm.Auth.CommandContextExtractor
  ...
  def handle_call({:add, tool_name, session_id, risk_level, agent_id, params}, _from, state) do
    ...
    if Process.whereis(Apm.Auth.ApprovalQueue) do
      Apm.Auth.ApprovalQueue.enqueue(entry)
    else
  ...
  defp notify_approval_required(entry) do
    ...
    agent_info =
      if Process.whereis(Apm.AgentRegistry) do
        case Apm.AgentRegistry.get_agent(entry.agent_id) do
