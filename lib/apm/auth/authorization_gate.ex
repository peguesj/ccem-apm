defmodule Apm.Auth.AuthorizationGate do
  @moduledoc """
  ...
  def authorize(agent_id, session_id, tool_name, role \\ "agent", params \\ %{}) do
    case verify_identity(agent_id, params) do
      {:ok, verified_agent_id, enriched_params} ->
        Apm.Instrumentation.span(
          [:apm, :auth, :authorization_gate, :evaluate],
  ...
  defp verify_identity(payload_agent_id, params) do
    ...
    case token do
      ...
      token when is_binary(token) ->
        case Apm.Auth.JwtAssertion.verify_assertion(token) do
          {:ok, claims} ->
  ...
  defp log_audit(event_type, resource, details) do
    try do
      ...
      Apm.AuditLog.log_with_context(event_type, "agentlock", resource, details, nil, context)
    rescue
      _ -> :ok
    catch
      :exit, _ -> :ok
  ...
  defp broadcast_decision(tool_name, status, risk_level, session_id) do
    Phoenix.PubSub.broadcast(Apm.PubSub, "apm:agentlock", %{
  ...
  defp broadcast(event) do
    Phoenix.PubSub.broadcast(Apm.PubSub, "agentlock:authorization", event)
