defmodule Apm.ActionEngine do
  ...
  defp execute_action("backfill_apm_config", project_path, _params) do
    ...
    config = %{
      ...
      session_id: Apm.Correlation.generate(),
  ...
  defp execute_action("discover_migrate_showcases", _project_path, _params) do
    config = Apm.ConfigLoader.get_config()
              ...
              true ->
                ...
                case copy_showcase_assets(client_dir, dest) do
                  {:ok, copied} ->
                    ...
                    Apm.ConfigLoader.update_project(updated_project)
                    Apm.ShowcaseDataStore.reload(name)
  ...
  defp execute_action("manage_showcases", _project_path, params) do
    ...
    case action do
      "list" ->
        projects = Apm.Showcases.ShowcaseManager.list_projects()
        stats = Apm.Showcases.ShowcaseManager.stats()
      ...
      "info" ->
        ...
        unless project do
          ...
          case Apm.Showcases.ShowcaseManager.get_project_showcase(project) do
            {:ok, info} ->
              {:ok,
               %{
      ...
      "create" ->
        ...
        unless project do
          ...
          case Apm.Showcases.ShowcaseManager.create_showcase(project, config) do
            {:ok, result} ->
              {:ok,
               %{
  ...
  defp execute_action("register_all_ports", _project_path, _params) do
    project_configs = Apm.PortManager.get_project_configs()
    ...
    results =
      Enum.map(project_configs, fn {project_name, config} ->
        ...
        if has_ports do
          ...
          case Apm.PortManager.assign_port(project_name) do
  ...
  defp execute_action("update_port_namespace", _project_path, %{
    ...
    unless Map.has_key?(ranges, namespace) do
      ...
      case Apm.PortManager.assign_port(namespace) do
        {:ok, new_port} ->
  ...
  defp execute_action("register_tool_permissions", _project_path, _params) do
    tools = Apm.Auth.PolicyEngine.default_risk_map()

    Enum.each(tools, fn {name, risk} ->
      Apm.Auth.AuthorizationGate.register_tool(name, risk)
    end)
  ...
  defp execute_action("audit_authorization_compliance", _project_path, _params) do
    summary = Apm.Auth.AuthorizationGate.summary()
    sessions = Apm.Auth.SessionStore.list_active()
  ...
  defp execute_action("manage_agent_lifecycle", _project_path, params) do
    ...
    if event do
      case Apm.Auth.AgentLifecycle.transition(:pending, event) do
  ...
  defp execute_action("lvm_integration_setup", _project_path, _params) do
    ...
    alias Apm.Integrations.IntegrationRegistry
