defmodule Apm.HookRegistry do
  ...
  def register_hook(hook) when is_map(hook) do
    ...
    Phoenix.PubSub.broadcast(Apm.PubSub, "apm:hooks", {:hook_registered, hook})
    :ok
  rescue
    ArgumentError -> :ok
  end
  ...
  def fire_hook(name, payload \\ %{}) do
    Phoenix.PubSub.broadcast(Apm.PubSub, "apm:hooks", {:hook_fired, name, payload})
    :ok
  rescue
    _ -> :ok
  end
