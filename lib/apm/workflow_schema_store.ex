defmodule Apm.WorkflowSchemaStore do
  @moduledoc """
  ...
  def handle_call({:update_phase, workflow_id, new_phase}, _from, s) do
    case :ets.lookup(@table, workflow_id) do
      [{^workflow_id, wf}] ->
        ...
        Phoenix.PubSub.broadcast(Apm.PubSub, "apm:workflows", {:workflow_phase_changed, updated})
        {:reply, {:ok, updated}, s}
  ...
  def handle_call({:update_workflow, workflow_id, attrs}, _from, s) do
    case :ets.lookup(@table, workflow_id) do
      [{^workflow_id, wf}] ->
        ...
        :ets.insert(@table, {workflow_id, updated})
        Phoenix.PubSub.broadcast(Apm.PubSub, "apm:workflows", {:workflow_updated, updated})
        {:reply, {:ok, updated}, s}
