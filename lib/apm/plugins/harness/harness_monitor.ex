...
defmodule Apm.Plugins.Harness.HarnessMonitor do
  @moduledoc """
  ...
  @impl true
  def init(_opts) do
    Phoenix.PubSub.subscribe(Apm.PubSub, "apm:worktrees")
  ...
  def handle_info(:poll, %{worktree_count: wt_count} = state) do
    ...
    if new_harness != state.harness do
      Phoenix.PubSub.broadcast(Apm.PubSub, @pubsub_topic, {:harness_state_updated, new_harness})
    end
