defmodule Apm.Plugins.Coalesce.CoalescePlugin do
  ...
  def plugin_description,
    do:
      "Skill logic coalescer — ingest sources, deploy formation, manage decision gates, apply refined skills"
  ...
  def list_endpoints do
    [
      %{
        action: "start",
        description: "Start a coalesce run",
        params: %{sources: "list", scope: "string", dry_run: "bool", auto_approve: "bool"}
      },
      ...
      %{
        action: "decide",
        description: "Approve/reject/defer a gate",
        params: %{
          run_id: "string",
          gate_id: "string",
      ...
      %{
        action: "apply",
        description: "Apply approved diffs for a run",
        params: %{run_id: "string"}
      },
      %{action: "cancel", description: "Cancel an in-progress run", params: %{run_id: "string"}},
      %{
        action: "preview",
        description: "Dry-run preview — formation plan + affected skills without starting a run",
        params: %{sources: "list", scope: "string"}
      }
    ]
  end
