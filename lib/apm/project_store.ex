defmodule Apm.ProjectStore do
  ...
  def sync_tasks(project_name, tasks) do
    ...
    Phoenix.PubSub.broadcast(Apm.PubSub, "apm:tasks", {:tasks_synced, project_name, tasks})
    :ok
  end
  ...
  def register_commands(project_name, commands) do
    ...
    Phoenix.PubSub.broadcast(Apm.PubSub, "apm:commands", {:commands_updated, project_name})
    :ok
  end
  ...
  def update_plane(project_name, plane_data) do
    ...
    Phoenix.PubSub.broadcast(Apm.PubSub, "apm:plane", {:plane_updated, project_name})
    :ok
  end
  ...
  def handle_call({:add_input_request, request}, _from, state) do
    ...
    Phoenix.PubSub.broadcast(Apm.PubSub, "apm:input", {:input_requested, input})
  ...
  def handle_call({:respond_to_input, id, choice}, _from, state) do
    case :ets.lookup(@input_table, id) do
      [{^id, input}] ->
        ...
        Phoenix.PubSub.broadcast(Apm.PubSub, "apm:input", {:input_responded, updated})
        {:reply, :ok, state}
