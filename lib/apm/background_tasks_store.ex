defmodule Apm.BackgroundTasksStore do
  @moduledoc """
  ...
  def handle_call({:register_task, attrs}, _from, state) do
    id = Map.get(attrs, "id") || Apm.Correlation.generate()
  ...
  def handle_call({:update_task, id, attrs}, _from, state) do
    case Map.get(state.tasks, id) do
      ...
      task ->
        ...
        Phoenix.PubSub.broadcast(Apm.PubSub, "tasks:updated", {:task_updated, updated})
        {:reply, :ok, new_state}
    end
  end
