defmodule Apm.HookHealthMonitor do
  @moduledoc """
  ...
  Scans `~/Developer/*/` (or the override from `Application.get_env(:apm, :hook_health_root)`)
  ...
  defp scan_projects do
    dev_root = Application.get_env(:apm, :hook_health_root, Path.expand("~/Developer"))
  ...
  defp broadcast_change(health, delta) do
    ...
    try do
      Phoenix.PubSub.broadcast(Apm.PubSub, @pubsub_topic, msg)
    rescue
      _ -> :ok
    catch
      _, _ -> :ok
