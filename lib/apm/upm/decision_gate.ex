defmodule Apm.Upm.DecisionGate do
  @moduledoc """
  ...
  def handle_call({:request, question, opts}, from, state) do
    ...
    Phoenix.PubSub.broadcast(Apm.PubSub, "upm:decisions", {:gate_created, gate})
  ...
  def handle_info({:gate_timeout, gate_id}, state) do
    case :ets.lookup(@table, gate_id) do
      [{^gate_id, %{status: :pending} = gate}] ->
        ...
        Phoenix.PubSub.broadcast(Apm.PubSub, "upm:decisions", {:gate_timeout, updated})
