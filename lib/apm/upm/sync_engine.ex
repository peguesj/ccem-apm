defmodule Apm.UPM.SyncEngine do
  @moduledoc """
  ...
  def handle_info(:sync_all, state) do
    ...
    Task.start(fn ->
      projects = Apm.UPM.ProjectRegistry.list_projects()
  ...
  defp do_sync_project(project_id) do
    ...
    integrations = Apm.UPM.PMIntegrationStore.list_for_project(project_id)
  ...
  defp sync_integration(integration, project_id) do
    adapter = Apm.UPM.Adapters.PMAdapter.get_adapter(integration.platform)
          ...
          {synced, drifted} =
            Enum.reduce(issues, {0, 0}, fn issue, {s, d} ->
              ...
              case Apm.UPM.WorkItemStore.upsert_item(attrs) do
                {:ok, item} ->
                  case Apm.UPM.WorkItemStore.detect_drift(item.id) do
                    {:ok, :synced} -> {s + 1, d}
                    {:ok, {:drift, _}} -> {s + 1, d + 1}
                    _ -> {s + 1, d}
                  end
  ...
  defp broadcast_result(result) do
    Phoenix.PubSub.broadcast(Apm.PubSub, @pubsub_topic, {:upm_sync_complete, result})
  end
end
