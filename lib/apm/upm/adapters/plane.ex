defmodule Apm.UPM.Adapters.Plane do
  @moduledoc """
  PM adapter for Plane project management platform.
  Delegates to Apm.PlaneClient for HTTP calls, normalizing responses
  to canonical WorkItem attribute maps.
  """
  @behaviour Apm.UPM.Adapters.PMAdapter
  ...
  def list_issues(integration) do
    ...
    Apm.PlaneClient.list_issues(project_id)
  rescue
  ...
  def create_issue(integration, attrs) do
    project_id = integration.project_key || integration.workspace
    Apm.PlaneClient.create_issue(project_id, attrs)
  rescue
    e -> {:error, "PlaneClient error: #{inspect(e)}"}
  end
  ...
  def update_issue(integration, issue_id, attrs) do
    project_id = integration.project_key || integration.workspace
    Apm.PlaneClient.update_issue(project_id, issue_id, attrs)
  rescue
    e -> {:error, "PlaneClient error: #{inspect(e)}"}
  end
  ...
  def test_connection(integration) do
    ...
    case Apm.PlaneClient.check_connection() do
