defmodule Apm.Identity.KeyStore do
  @moduledoc """
  ...
  Registered under `Apm.Identity.KeyStore` by default (overridable via
  `name:` option) and wired into `Apm.Application`.
  ...
  defp resolve_key_file(opts) do
    case Keyword.get(opts, :key_file) do
      nil ->
        env = Application.get_env(:apm, :env, Mix.env() |> to_string())
