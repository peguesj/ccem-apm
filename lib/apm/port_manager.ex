defmodule Apm.PortManager do
  ...
  def handle_call(:get_project_configs, _from, state) do
    ...
    apm_config = Apm.ConfigLoader.get_config()
  ...
  def handle_cast({:scan_result, active}, state) do
    ...
    try do
      Phoenix.PubSub.broadcast(Apm.PubSub, "apm:ports", {:ports_updated, active})
    rescue
      _ -> :ok
    end
    ...
    if is_nil(state.last_scan) do
      try do
        Phoenix.PubSub.broadcast(Apm.PubSub, "apm:boot", {:port_scan_complete, map_size(active)})
      rescue
        _ -> :ok
      catch
        :exit, _ -> :ok
  ...
  defp scan_interval_ms do
    Application.get_env(:apm, :port_scan_interval, @default_scan_interval_ms)
  end
  ...
  defp build_recommendation(claimants, alternatives) do
    ...
    exclusive_owner =
      Enum.find(claimants, fn c ->
        project = Apm.ConfigLoader.get_project(c.project)
