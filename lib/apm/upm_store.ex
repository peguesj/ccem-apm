defmodule Apm.UpmStore do
  ...
  def register_formation(params) do
    ...
    Phoenix.PubSub.broadcast(Apm.PubSub, "apm:upm", {:formation_registered, formation})
  ...
  def list_all_formations do
    ...
    notif_only =
      Apm.AgentRegistry.get_notifications()
      |> Enum.filter(&(Map.get(&1, :formation_id) not in [nil, ""]))
      |> Enum.group_by(&Map.get(&1, :formation_id))
  ...
  def handle_call({:register_session_design_handoff, params, parsed}, _from, state) do
    ...
    Phoenix.PubSub.broadcast(Apm.PubSub, "apm:upm", {:upm_session_registered, session})
    ...
    case :ets.lookup(@sessions_table, session_id) do
      [{^session_id, session}] ->
        ...
        if agent_id do
          ...
          Apm.AgentRegistry.register_agent(agent_id, metadata)
        end

        Phoenix.PubSub.broadcast(Apm.PubSub, "apm:upm", {:upm_agent_registered, params})
  ...
  def handle_call({:record_event, params}, _from, state) do
    ...
    Phoenix.PubSub.broadcast(Apm.PubSub, "apm:upm", {:upm_event, event})
  ...
  defp persist_event(formation_id, event_type, payload) do
    if Process.whereis(Apm.Orchestration.FormationPersistenceStore) do
      Apm.Orchestration.FormationPersistenceStore.append_event(
        formation_id,
        event_type,
        payload
      )
