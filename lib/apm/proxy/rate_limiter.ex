defmodule Apm.Proxy.RateLimiter do
  @moduledoc """
  Proxy rate limiter — thin compatibility wrapper over `Apm.RateLimit`.
  ...
  table and cleanup scheduler.  `Apm.Proxy.Supervisor` replaces the
  `Apm.Proxy.RateLimiter` child with `Apm.RateLimit`.
  """
  ...
  def allow?(scope, key) do
    ...
    case Apm.RateLimit.hit(bucket, @default_window_ms, @default_limit) do
      {:allow, _count} -> true
  ...
  def check(scope, key) do
    ...
    case Apm.RateLimit.hit(bucket, @default_window_ms, 999_999_999) do
      {:allow, count} ->
  ...
  achieved by restarting `Apm.RateLimit`.
  """
  @spec reset(term()) :: :ok
