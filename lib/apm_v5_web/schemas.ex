defmodule ApmV5Web.Schemas do
  ...
  defmodule PaginatedAgents do
    ...
    OpenApiSpex.schema(%{
      ...
      type: :object,
      properties: %{
        data: %Schema{type: :array, items: ApmV5Web.Schemas.LegacyAgent},
        meta: %Schema{
          type: :object,
          properties: %{
            total: %Schema{type: :integer},
  ...
  defmodule ApprovalRequest do
    ...
    OpenApiSpex.schema(%{
      ...
      properties: %{
        ...
        risk_level: %Schema{type: :string, enum: ["low", "medium", "high", "critical"]},
        status: %Schema{type: :string, enum: ["pending", "approved", "rejected", "timeout"]},
        execution_context: ApmV5Web.Schemas.ExecutionContext,
        display_name: %Schema{
          type: :string,
          nullable: true,
  ...
  defmodule ApprovalAuditEntry do
    ...
    OpenApiSpex.schema(%{
      ...
      properties: %{
        ...
        timestamp: %Schema{type: :string, format: :"date-time"},
        decision: %Schema{type: :string, enum: ["approved", "rejected", "timeout"]},
        context_snapshot: ApmV5Web.Schemas.ExecutionContext,
        reason: %Schema{type: :string, nullable: true},
        method: %Schema{
          type: :string,
