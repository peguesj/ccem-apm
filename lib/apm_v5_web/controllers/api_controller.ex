defmodule ApmV5Web.ApiController do
  ...
  def health(conn, _params) do
    payload = StatusCache.fetch(:health_payload, &build_health_payload/0)
    json(conn, payload)
  end

  @doc "GET /api/status -- existing v4 status endpoint (includes project data for CCEMHelper)"
  def status(conn, _params) do
    payload = StatusCache.fetch(:status_payload, &build_status_payload/0)
    json(conn, payload)
  end

  # Builds /api/status response. Runs at most once per cache TTL.
  # Uses a single-pass agent map instead of N × :ets.tab2list to avoid O(projects × agents).
  ...
  # Single O(agents) pass to build {project_name => agent_count} map.
  # Agents with nil project_name count toward every project (matches legacy semantics
  # of `list_agents(project_name)` which includes nil-project agents).
  ...
  def update_project(conn, params) do
    case ConfigLoader.update_project(params) do
      {:ok, config} ->
        Phoenix.PubSub.broadcast(ApmV5.PubSub, "apm:config", {:config_reloaded, config})
        json(conn, %{status: "ok"})
  ...
  def export(conn, params) do
    case Map.get(params, "format") do
      "csv" ->
        ...
        case ApmV5.ExportManager.export_csv(section) do
          {:error, reason} ->
      ...
      _ ->
        ...
        data = ApmV5.ExportManager.export(opts)
        json(conn, data)
    end
  end
  ...
  def import_data(conn, params) do
    case ApmV5.ExportManager.import(params) do
      {:ok, summary} ->
  ...
  def assign_port(conn, %{"project" => project}) do
    case ApmV5.PortManager.assign_port(project) do
      {:ok, port} -> json(conn, %{ok: true, port: port})
      {:error, reason} -> conn |> put_status(422) |> json(%{ok: false, error: to_string(reason)})
    end
  end
  ...
  def set_primary_port(conn, %{"project" => project, "port" => port} = params) do
    ...
    if ownership not in ["exclusive", "shared", "reserved"] do
      ...
      case ApmV5.PortManager.set_primary_port(project, port, ownership) do
        :ok ->
  ...
  def deploy_hooks(conn, %{"skill" => skill, "project_root" => project_root} = params) do
    ...
    case ApmV5.SkillHookDeployer.deploy_hooks(project_root, skill, hooks) do
      {:ok, result} ->
  ...
  def list_bg_tasks(conn, params) do
    ...
    tasks = ApmV5.BackgroundTasksStore.list_tasks(filter)
    json(conn, %{tasks: tasks})
  end

  def register_bg_task(conn, params) do
    case ApmV5.BackgroundTasksStore.register_task(params) do
  ...
  def get_bg_task(conn, %{"id" => id}) do
    case ApmV5.BackgroundTasksStore.get_task(id) do
      {:ok, task} -> json(conn, %{task: task})
  ...
  def get_bg_task_logs(conn, %{"id" => id} = params) do
    ...
    case ApmV5.BackgroundTasksStore.get_task_logs(id, lines) do
      {:ok, log_lines} ->
        log_path =
          case ApmV5.BackgroundTasksStore.get_task(id) do
  ...
  def update_bg_task(conn, %{"id" => id} = params) do
    ...
    case ApmV5.BackgroundTasksStore.update_task(id, attrs) do
  ...
  def stop_bg_task(conn, %{"id" => id}) do
    case ApmV5.BackgroundTasksStore.get_task(id) do
      {:ok, task} ->
        ...
        ApmV5.BackgroundTasksStore.update_task(id, %{"status" => "stopping"})
  ...
  def delete_bg_task(conn, %{"id" => id}) do
    ApmV5.BackgroundTasksStore.delete_task(id)
    json(conn, %{ok: true})
  end
  ...
  def scanner_scan(conn, params) do
    ...
    case ApmV5.ProjectScanner.scan(base_path) do
  ...
  def list_actions(conn, _params) do
    catalog = ApmV5.ActionEngine.list_catalog()
  ...
  def run_action(conn, params) do
    ...
    case ApmV5.ActionEngine.run_action(action_type, project_path, action_params) do
  ...
  def list_action_runs(conn, _params) do
    runs = ApmV5.ActionEngine.list_runs()
    json(conn, %{runs: runs})
  end

  def get_action_run(conn, %{"id" => id}) do
    case ApmV5.ActionEngine.get_run(id) do
  ...
  def intake_submit(conn, params) do
    case ApmV5.Intake.Store.submit(params) do
      {:ok, event} ->
  ...
  def intake_list(conn, params) do
    ...
    events = ApmV5.Intake.Store.list(opts)
  ...
  def intake_watchers(conn, _params) do
    watchers = ApmV5.Intake.Store.watchers()
    json(conn, %{
      ok: true,
