defmodule ApmV4.VerifyStore do
  @moduledoc """
  ...
  def handle_call({:create, project_root, app_url, stories}, _from, state) do
    ...
    Phoenix.PubSub.broadcast(ApmV4.PubSub, "apm:verify", {:verify_created, session})
  ...
  def handle_call({:update, id, attrs}, _from, state) do
    case :ets.lookup(@table, id) do
      [{^id, session}] ->
        ...
        Phoenix.PubSub.broadcast(ApmV4.PubSub, "apm:verify", {:verify_updated, updated})
