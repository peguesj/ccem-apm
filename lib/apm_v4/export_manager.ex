defmodule ApmV4.ExportManager do
  @moduledoc """
  ...
  defp export_agents(nil) do
    ApmV4.AgentRegistry.list_agents()
    |> sanitize_list()
  end
  ...
  defp export_sessions do
    ApmV4.AgentRegistry.list_sessions()
    |> sanitize_list()
  end

  defp export_metrics do
    ApmV4.MetricsCollector.get_fleet_metrics()
  end

  defp export_slos do
    ApmV4.SloEngine.get_all_slis()
    |> Enum.map(fn sli -> Map.delete(sli, :recent_events) end)
  end

  defp export_alert_rules do
    ApmV4.AlertRulesEngine.list_rules()
  end

  defp export_alert_history(nil) do
    ApmV4.AlertRulesEngine.get_alert_history(limit: 1000)
    |> Enum.map(&stringify_datetimes/1)
  end

  defp export_alert_history(%DateTime{} = since) do
    ApmV4.AlertRulesEngine.get_alert_history(limit: 1000, since: since)
    |> Enum.map(&stringify_datetimes/1)
  end

  defp export_audit_log(nil) do
    ApmV4.AuditLog.query(limit: 1000)
  end

  defp export_audit_log(%DateTime{} = since) do
    since_str = DateTime.to_iso8601(since)
    ApmV4.AuditLog.query(limit: 1000, since: since_str)
  end
  ...
  defp import_agents(%{agents: agents}) when is_list(agents) do
    Enum.each(agents, fn agent ->
      ...
      if id, do: ApmV4.AgentRegistry.register_agent(id, agent)
    end)
  ...
  defp import_agents(%{"agents" => agents}) when is_list(agents) do
    Enum.each(agents, fn agent ->
      id = Map.get(agent, :id, Map.get(agent, "id"))
      if id, do: ApmV4.AgentRegistry.register_agent(id, atomize_keys(agent))
    end)
  ...
  defp import_alert_rules(%{alert_rules: rules}) when is_list(rules) do
    Enum.each(rules, fn rule ->
      ApmV4.AlertRulesEngine.add_rule(rule)
    end)
  ...
  defp import_alert_rules(%{"alert_rules" => rules}) when is_list(rules) do
    Enum.each(rules, fn rule ->
      ApmV4.AlertRulesEngine.add_rule(atomize_keys(rule))
    end)
