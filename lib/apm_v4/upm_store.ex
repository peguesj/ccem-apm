defmodule ApmV4.UpmStore do
  ...
  def register_formation(params) do
    ...
    Phoenix.PubSub.broadcast(ApmV4.PubSub, "apm:upm", {:formation_registered, formation})
    {:ok, id}
  end
  ...
  def update_formation(formation_id, fields) do
    case :ets.lookup(@formations_table, formation_id) do
      [{^formation_id, f}] ->
        ...
        Phoenix.PubSub.broadcast(ApmV4.PubSub, "apm:upm", {:formation_updated, updated})
        :ok
      [] -> {:error, :not_found}
    end
  end
  ...
  def handle_call({:register_session, params}, _from, state) do
    ...
    Phoenix.PubSub.broadcast(ApmV4.PubSub, "apm:upm", {:upm_session_registered, session})
    ...
    case :ets.lookup(@sessions_table, session_id) do
      [{^session_id, session}] ->
        ...
        if agent_id do
          ...
          ApmV4.AgentRegistry.register_agent(agent_id, metadata)
        end

        Phoenix.PubSub.broadcast(ApmV4.PubSub, "apm:upm", {:upm_agent_registered, params})
  ...
  def handle_call({:record_event, params}, _from, state) do
    ...
    Phoenix.PubSub.broadcast(ApmV4.PubSub, "apm:upm", {:upm_event, event})
