defmodule ApmV5.Auth.AdaptiveRateLimiter do
  @moduledoc """
  ...
  queue length of `ApmV5.AgentRegistry` as a representative load signal for the
  ...
  Callers import just `ApmV5.Auth.AdaptiveRateLimiter.adaptive_factor/0`; this
  GenServer manages state internally.

  ## Telemetry

  Emits `[:apm_v5, :rate_limiter, :adaptive, :scaled]` with
  ...
  defp apply_factor(factor, _old_factor, queue_len) do
    ...
    :telemetry.execute(
      [:apm_v5, :rate_limiter, :adaptive, :scaled],
