defmodule ApmV5.Coalesce.DecisionGateStore do
  @moduledoc """
  ...
  def handle_info(:expire_check, state) do
    ...
    |> Enum.each(fn gate ->
      ...
      if age_ms > gate.timeout_ms do
        ...
        # Notify orchestrator
        send(ApmV5.Coalesce.CoalesceOrchestrator, {:gate_decided, gate.run_id, gate.gate_id, :expired})
      end
    end)
  ...
  defp _transition(composite_id, new_status, extra, state) do
    case :ets.lookup(@table, composite_id) do
      [{^composite_id, %{status: :pending} = gate}] ->
        ...
        # Notify orchestrator
        send(ApmV5.Coalesce.CoalesceOrchestrator,
