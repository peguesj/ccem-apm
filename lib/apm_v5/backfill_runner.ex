defmodule ApmV5.BackfillRunner do
  @moduledoc """
  ...
  def run_primary do
    ...
    ApmV5.BackfillStore.add_run(%{status: :running, started_at: started_at, message: "Starting backfill..."})

    result = do_backfill()

    run = Map.merge(result, %{started_at: started_at, completed_at: DateTime.utc_now()})
    ApmV5.BackfillStore.add_run(run)
    run
  end
  ...
  def check_api_connection do
    :inets.start()
    :ssl.start()
    ApmV5.PlaneClient.check_connection()
  end

  # --- Private ---

  defp do_backfill do
    with {:ok, prd} <- ApmV5.PrdScanner.find_primary(),
         {:ok, stories} <- read_stories(prd.path),
         {:ok, issues} <- ApmV5.PlaneClient.list_issues(@ccem_project_id) do
      ...
      case matching_issue do
        ...
        issue ->
          ...
          if current_state == state_id do
            ...
            case ApmV5.PlaneClient.update_issue(@ccem_project_id, issue_id, %{"state" => state_id}) do
              {:ok, _} ->
