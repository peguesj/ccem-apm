defmodule ApmV5.UPM.Adapters.Plane do
  @moduledoc """
  PM adapter for Plane project management platform.
  Delegates to ApmV5.PlaneClient for HTTP calls, normalizing responses
  to canonical WorkItem attribute maps.
  """
  @behaviour ApmV5.UPM.Adapters.PMAdapter
  ...
  def list_issues(integration) do
    ...
    ApmV5.PlaneClient.list_issues(project_id)
  rescue
  ...
  def create_issue(integration, attrs) do
    project_id = integration.project_key || integration.workspace
    ApmV5.PlaneClient.create_issue(project_id, attrs)
  rescue
    e -> {:error, "PlaneClient error: #{inspect(e)}"}
  end
  ...
  def update_issue(integration, issue_id, attrs) do
    project_id = integration.project_key || integration.workspace
    ApmV5.PlaneClient.update_issue(project_id, issue_id, attrs)
  rescue
    e -> {:error, "PlaneClient error: #{inspect(e)}"}
  end
  ...
  def test_connection(integration) do
    ...
    case ApmV5.PlaneClient.check_connection() do
