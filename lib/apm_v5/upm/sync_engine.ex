defmodule ApmV5.UPM.SyncEngine do
  @moduledoc """
  ...
  def handle_info(:sync_all, state) do
    ...
    Task.start(fn ->
      projects = ApmV5.UPM.ProjectRegistry.list_projects()
  ...
  defp do_sync_project(project_id) do
    ...
    integrations = ApmV5.UPM.PMIntegrationStore.list_for_project(project_id)
  ...
  defp sync_integration(integration, project_id) do
    adapter = ApmV5.UPM.Adapters.PMAdapter.get_adapter(integration.platform)
        ...
        {:ok, issues} ->
          {synced, drifted} =
            Enum.reduce(issues, {0, 0}, fn issue, {s, d} ->
              ...
              case ApmV5.UPM.WorkItemStore.upsert_item(attrs) do
                {:ok, item} ->
                  case ApmV5.UPM.WorkItemStore.detect_drift(item.id) do
                    {:ok, :synced} -> {s + 1, d}
                    {:ok, {:drift, _}} -> {s + 1, d + 1}
                    _ -> {s + 1, d}
                  end
  ...
  defp broadcast_result(result) do
    Phoenix.PubSub.broadcast(ApmV5.PubSub, @pubsub_topic, {:upm_sync_complete, result})
  end
end
