defmodule ApmV5.Governance.GovernanceKriPoller do
  @moduledoc """
  Periodic GenServer that emits the `[:apm_v5, :governance, :risk_score_p95]`
  ...
  `ApmV5.Auth.Types.risk_severity/1` and then calculating the 95th percentile
  ...
  ## Telemetry event
      ...
      [:apm_v5, :governance, :risk_score_p95]
      measurements: %{value: float()}   # 0.0 – 4.0
      metadata:     %{sample_size: non_neg_integer()}

  ## Prometheus metric

  Registered in `ApmV5Web.Telemetry` as a `last_value` → becomes
