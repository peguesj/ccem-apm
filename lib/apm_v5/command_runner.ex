defmodule ApmV5.CommandRunner do
  @moduledoc """
  ...
  def handle_call({:exec, env_name, command, opts}, _from, state) do
    ...
    if dangerous?(command) do
      ...
      case ApmV5.EnvironmentScanner.get_environment(env_name) do
        {:ok, env} ->
  ...
  defp run_command(command, cwd, timeout, topic, request_id) do
    ...
    case Task.yield(task, timeout) || Task.shutdown(task, :brutal_kill) do
      ...
      nil ->
        Phoenix.PubSub.broadcast(ApmV5.PubSub, topic, {:output, "[timeout after #{div(timeout, 1000)}s]\n"})
  ...
  defp collect_output(port, acc, topic) do
    receive do
      {^port, {:data, data}} ->
        Phoenix.PubSub.broadcast(ApmV5.PubSub, topic, {:output, data})
