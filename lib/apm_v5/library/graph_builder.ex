defmodule ApmV5.Library.GraphBuilder do
  @moduledoc """
  Builds a relationship graph across the CCEM ecosystem by reading
  `ApmV5.LibraryStore` data (7 categories: agents, skills, mcp_servers, tools,
  ...
  * `:types` - limit to subset of node types
  ...
  defp fetch_library_data do
    if ets_available?() do
      %{
        skills: safe_list(&ApmV5.LibraryStore.list_skills/0),
        agents: safe_list(&ApmV5.LibraryStore.list_agents/0),
        commands: safe_list(&ApmV5.LibraryStore.list_commands/0),
        tools: safe_list(&ApmV5.LibraryStore.list_tools/0),
        patterns: safe_list(&ApmV5.LibraryStore.list_patterns/0),
        learnings: safe_list(&ApmV5.LibraryStore.list_learnings/0),
        mcp_servers: safe_list(&ApmV5.LibraryStore.list_mcp_servers/0)
      }
    else
