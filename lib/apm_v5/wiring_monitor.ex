defmodule ApmV5.WiringMonitor do
  @moduledoc """
  ...
  Each check returns a list of `%ApmV5.WiringMonitor.Finding{}` structs.
  ...
  @doc """
  W1: Walk `ApmV5Web.Router.__routes__/0` and verify each target module
  ...
  `lib/apm_v5_web/` against the set of keys registered in `assets/js/app.js`.
  ...
  @spec routes() :: [map()]
  def routes do
    if Code.ensure_loaded?(ApmV5Web.Router) and
         function_exported?(ApmV5Web.Router, :__routes__, 0) do
  ...
  def all_live_modules do
    {:ok, mods} = :application.get_key(:apm_v5, :modules)

    mods
    |> Enum.filter(fn mod ->
      name = to_string(mod)
      String.contains?(name, "ApmV5Web") and
  ...
  def registered_hooks do
    app_js_path =
      Application.app_dir(:apm_v5, "priv/static/assets/app.js")
  ...
  @doc """
  Hook names actually referenced in HEEx templates under `lib/apm_v5_web/`.
  """
  @spec emitted_hooks() :: MapSet.t(String.t())
  def emitted_hooks do
    web_dir = Path.join([File.cwd!(), "lib", "apm_v5_web"])
  ...
  @doc """
  Topics subscribed via `Phoenix.PubSub.subscribe(ApmV5.PubSub, topic)` across the codebase.
  ...
  @doc """
  Topics broadcast via `Phoenix.PubSub.broadcast(ApmV5.PubSub, topic, ...)` across the codebase.
  Extracted statically — returns the hard-coded literal set used at analysis time.
  """
