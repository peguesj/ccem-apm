defmodule ApmV5.ConcurrencyLayer do
  @moduledoc """
  ...
  this module is:
    ...
    * supervised by `ApmV5.ConcurrencyLayer.TaskSupervisor`
  ...
  Events emitted:
    ...
    * `[:apm_v5, :concurrency, :task, :start]`
    * `[:apm_v5, :concurrency, :task, :stop]`  -- measurement `:duration` (native time)
    * `[:apm_v5, :concurrency, :task, :exception]`

  Metadata: `%{label: atom()}`.
  """

  @task_supervisor ApmV5.ConcurrencyLayer.TaskSupervisor
  ...
  defp run_with_telemetry(fun, label) do
    ...
    meta = %{label: label}
    :telemetry.execute([:apm_v5, :concurrency, :task, :start], %{system_time: System.system_time()}, meta)

    try do
      result = fun.()
      duration = System.monotonic_time() - start
      :telemetry.execute([:apm_v5, :concurrency, :task, :stop], %{duration: duration}, meta)
      result
    rescue
      e ->
        duration = System.monotonic_time() - start
