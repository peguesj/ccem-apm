defmodule ApmV5.NativeTransport.UnixSocket do
  @moduledoc """
  ...
  ## Lifecycle
    ...
    * Each connected client is tracked under `ApmV5.NativeTransport.ConnectionRegistry`
      so `ChannelBridge` (Wave 2) can push to specific subscribers.

  ## Telemetry

    * `[:apm_v5, :native_transport, :connection, :accept]`
    * `[:apm_v5, :native_transport, :connection, :close]`
    * `[:apm_v5, :native_transport, :frame, :recv]` -- measurement `:bytes`
    * `[:apm_v5, :native_transport, :frame, :send]` -- measurement `:bytes`
  """
  ...
  def send_frame(socket, term) do
    ...
    :telemetry.execute(
      [:apm_v5, :native_transport, :frame, :send],
  ...
  def recv_frame(socket, timeout \\ 5_000) do
    with {:ok, <<size::32-big>>} <- :gen_tcp.recv(socket, 4, timeout),
      ...
      :telemetry.execute(
        [:apm_v5, :native_transport, :frame, :recv],
        %{bytes: 4 + size},
        %{}
      )
      ...
      {:ok, client} ->
        ...
        {:ok, _pid} =
          Task.Supervisor.start_child(
            ApmV5.ConcurrencyLayer.TaskSupervisor,
  ...
  defp handle_client_frame(socket, {:hello, client_info}) do
    send_frame(socket, {:hello_ack, %{server: "apm_v5", ts: System.system_time(:millisecond), client: client_info}})
  end
