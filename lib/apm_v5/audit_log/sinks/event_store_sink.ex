defmodule ApmV5.AuditLog.Sinks.EventStoreSink do
  @moduledoc """
  ...
  replaced via config:
      ...
      config :apm_v5, :event_store_adapter_fn, fn stream, version, events, opts ->
  ...
  Implements `ApmV5.AuditLog.Sink`. Return `:ok` on success, `{:error, reason}`
  ...
  def backend_enabled? do
    Application.get_env(:apm_v5, :audit_backend, :ets) == :eventstore
  end
  ...
  defp resolve_adapter do
    Application.get_env(:apm_v5, :event_store_adapter_fn) ||
      &default_adapter/4
  end
