defmodule ApmV4Web.V2.AgentControlController do
  ...
  def list_messages(conn, %{"id" => agent_id} = params) do
    ...
    messages = ApmV4.ChatStore.list_messages(scope, limit)
  ...
  def send_message(conn, %{"id" => agent_id, "content" => content} = params) do
    ...
    case ApmV4.ChatStore.send_message(scope, content, metadata) do
      {:ok, message} ->
  ...
  defp execute_control(agent_id, action, _agent) do
    ...
    try do
      ApmV4.AgentRegistry.update_agent(agent_id, %{status: new_status})
    rescue
      _ -> :ok
    end
    ...
    try do
      ...
      ApmV4.EventStream.emit(event_type, %{
        agent_id: agent_id,
        action: action,
