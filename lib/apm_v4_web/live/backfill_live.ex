defmodule ApmV4Web.BackfillLive do
  ...
  def handle_event("sync_to_plane", _params, socket) do
    ...
    Task.start(fn ->
      ApmV4.BackfillRunner.run_primary()
    end)
    {:noreply, socket}
  end
  ...
  def handle_event("check_api", _params, socket) do
    result = ApmV4.BackfillRunner.check_api_connection()
  ...
  def handle_event("insert_rule", _params, socket) do
    result = ApmV4.UpmPersistentRule.insert_rule()
    message = case result do
      {:ok, msg} -> msg
      {:error, msg} -> "Error: #{msg}"
    end
  ...
  defp assign_data(socket) do
    store = ApmV4.BackfillStore.get_state()
    prd_files = ApmV4.PrdScanner.scan()
    rule_status = ApmV4.UpmPersistentRule.check_rule()
    assign(socket,
      store: store,
      prd_files: prd_files,
